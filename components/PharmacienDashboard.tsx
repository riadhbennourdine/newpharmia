...
const PharmacienDashboard: React.FC = () => {
  ...
  const [viewingPreparator, setViewingPreparator] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const fetchPreparators = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/users/groups/${group._id}/preparateurs`,
        {
          headers: {
    ...
    const fetchInstructionFiches = async () => {
      ...
      try {
        const responses = await Promise.all(
          ficheIdsToFetch.map((ficheId) => fetch(`/api/memofiches/${ficheId}`)),
        );
    ...
    return (
      <div className="text-center p-8 bg-white rounded-lg shadow-md">
        ...
        <p className="text-slate-600">
          Il semble que vous ne soyez pas encore associé à un groupe. Veuillez
        ...
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 text-center mb-6 transition-transform duration-300 hover:scale-105 hover:shadow-xl">
            ...
            <p className="text-slate-700 font-semibold text-base">
              {group?.instruction || 'Aucune consigne pour le moment.'}
            </p>
            ...
            <button
              ...
              className="text-sm text-teal-600 hover:text-teal-800 font-semibold mt-4"
            >
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 border border-teal-100 overflow-hidden relative">
              <div className="relative z-10">
                <p className="text-slate-700 font-semibold text-lg mb-4">
                  Votre Coach PharmIA d'apprentissage au quotidien
                </p>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-sm text-teal-600 font-medium">
                          Voir comment l'IA teste vos équipes
                        </p>
                      </div>
                    </div>
                    <svg
