...
const EditInstructionModal: React.FC<Props> = ({ group, onClose, setInstruction }) => {
    ...
    const [primaryMemoFicheId, setPrimaryMemoFicheId] = useState<string>(group?.primaryMemoFicheId?.toString() || '');
    const [additionalMemoFicheIds, setAdditionalMemoFicheIds] = useState<(string | ObjectId)[]>(group?.instructionFiches || []);
        ...
        try {
            const response = await fetch(`/api/groups/${group._id}/instruction`, {
                ...
                body: JSON.stringify({
                    ...
                    additionalMemoFicheIds: additionalMemoFicheIds.map(id => id.toString()),
                }),
            });
            if (response.ok) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
                ...
                />
                <div className="mt-4">
                    <label htmlFor="primaryMemofiche" className="block text-sm font-medium text-gray-700">Mémofiche principale (optionnel)</label>
                    <select
                        id="primaryMemofiche"
                ...
                <div className="mt-4">
                    <label htmlFor="additionalMemofiches" className="block text-sm font-medium text-gray-700">Mémofiches additionnelles (optionnel)</label>
                    <input
                        type="text"
                    ...
                    {searchMemoficheTerm && (
                        <div className="border border-gray-300 rounded-md mt-1 max-h-40 overflow-y-auto">
                            {memofiches
                                .filter(fiche => 
                                    fiche.title.toLowerCase().includes(searchMemoficheTerm.toLowerCase()) &&
                                ...
                                .map(fiche => (
                                    <div
                                        ...
                                        onClick={() => {
                                            setAdditionalMemoFicheIds(prev => [...prev, fiche._id as string]);
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                        {additionalMemoFicheIds.map(ficheId => {
                            ...
                            return fiche ? (
                                <span key={ficheId as string} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-100 text-teal-800">
                                    {fiche.title}
                                    <button
                                        type="button"
                                        onClick={() => setAdditionalMemoFicheIds(prev => prev.filter(id => id !== ficheId))}
