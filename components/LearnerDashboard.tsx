...
const LearnerDashboard: React.FC<Props> = ({ initialGroup }) => {
  ...
  const [primaryFicheDetails, setPrimaryFicheDetails] =
    useState<CaseStudy | null>(null);
  const [additionalFicheDetails, setAdditionalFicheDetails] = useState<
    CaseStudy[]
  >([]);
  const [validReadFichesCount, setValidReadFichesCount] = useState(
    user?.readFiches?.length || 0,
  );
  ...
  const [planningDetails, setPlanningDetails] = useState<{
    active: (GroupAssignment & { title: string })[];
    upcoming: (GroupAssignment & { title: string })[];
  }>({ active: [], upcoming: [] });
  ...
  useEffect(() => {
    const fetchInstructionFiches = async () => {
      ...
      if (group.instructionFiches && group.instructionFiches.length > 0) {
        ficheIdsToFetch.push(
          ...group.instructionFiches.map((id) => id.toString()),
        );
      }
      ...
      try {
        ...
        if (group.primaryMemoFicheId) {
          const primary = data.find(
            (d) => d._id === group.primaryMemoFicheId.toString(),
          );
  ...
  const averageSimulationScore =
    simulationsRealisees > 0
      ? Math.round(
          simulationHistory.reduce((acc, sim) => acc + (sim.score || 0), 0) /
            simulationsRealisees,
        )
      : 0;
    ...
    <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <h3 className="text-lg font-semibold text-slate-700 mb-3 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-green-500"></span>À
                étudier maintenant
              </h3>
              {planningDetails.active.length > 0 ? (
                    ...
                    <li key={idx}>
                      ...
                      >
                        <div className="flex justify-between items-center">
                          <span className="font-bold text-teal-800 group-hover:text-teal-600">
                            {item.title}
                          </span>
              ...
              {planningDetails.upcoming.length > 0 ? (
                <ul className="space-y-3">
                  {planningDetails.upcoming.map((item, idx) => (
                    <li
                      key={idx}
                      className="block p-3 bg-slate-50 rounded-lg border border-slate-200 opacity-75"
                    >
                      <div className="flex justify-between items-center">
      ...
      {group?.instruction && (
        <div className="bg-white rounded-xl shadow-lg p-6 text-center mb-6 transition-transform duration-300 hover:scale-105 hover:shadow-xl">
          ...
          {group.instructionDate && (
            <p className="text-sm text-gray-500 mt-2">
              Donnée le:{' '}
              {new Date(group.instructionDate).toLocaleDateString('fr-FR')}
            </p>
          )}
          ...
          {primaryFicheDetails && (
            <div className="mt-4">
              ...
              <Link
                to={`/memofiche/${primaryFicheDetails._id}`}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <h3 className="font-bold text-teal-800 text-xl mb-1">
                      Réviser avec mon Coach
                    </h3>
                    <p className="text-sm text-teal-600 font-medium">
                      Teste tes connaissances sur les fiches
                    </p>
                  </div>
                </div>
                <svg
                  ...
                  strokeWidth={2.5}
                  stroke="currentColor"
                  className="w-6 h-6 text-teal-400 group-hover:text-teal-600 group-hover:translate-x-1 transition-all"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
      ...
      {/* Modal Compagnon IA */}
      {showCompagnon && user && (
        <CompagnonIA user={user} onClose={() => setShowCompagnon(false)} />
      )}
    </>
  );
};
