...
const TeamBriefingPlayer: React.FC = () => {
    ...
    const fetchExistingBriefing = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          setErrorMessage(
            errorData.message || 'Erreur lors de la récupération du briefing.',
          );
        ...
        setErrorMessage(
          error.message || 'Erreur lors de la récupération du briefing.',
        );
      }
    };
  ...
  const generateBriefing = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        setErrorMessage(
          errorData.message || 'Erreur lors de la génération du briefing.',
        );
        return; // Stop processing if response is not ok
      }
      ...
      setErrorMessage(
        error.message || 'Erreur lors de la génération du briefing.',
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const speakChunks = (chunks: string[], index: number) => {
    ...
    if (language === 'ar') {
      ...
      bestVoice =
        voices.find(
          (v) => v.name.includes('Google') && v.lang.includes('fr'),
        ) ||
        voices.find(
          (v) => v.name.includes('Thomas') && v.lang.includes('fr'),
        ) ||
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex-1 text-center md:text-left">
          <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
            <span className="bg-teal-100 text-teal-800 px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wider">
              Flash Info
            </span>
          ...
          {instruction && (
            <div className="mt-4 p-3 bg-teal-50 border border-teal-100 rounded-lg">
              <p className="text-xs font-bold text-teal-800 uppercase tracking-tight mb-1">
                Consigne du Titulaire :
              </p>
              ...
              <div className="flex items-center gap-4 bg-slate-100 p-2 pr-6 rounded-full animate-fadeIn">
                ...
                <div className="flex flex-col">
                  <span className="text-xs font-medium text-slate-500 uppercase">
                    {isScriptToday ? "Aujourd'hui" : 'Ancien'}
                  </span>
