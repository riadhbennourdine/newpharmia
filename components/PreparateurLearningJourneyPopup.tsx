...
const PreparerLearningJourneyPopup: React.FC<
  PreparerLearningJourneyPopupProps
> = ({ preparerId, preparerName, onClose, startDate, endDate }) => {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [learningJourney, setLearningJourney] =
    useState<LearningJourneyData | null>(null);
    ...
    const fetchLearningJourney = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/users/${preparerId}/learning-journey?${params.toString()}`,
          {
            headers: {
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl max-w-3xl w-full relative max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-2">
          Parcours d'apprentissage de{' '}
          <span className="text-teal-600">{preparerName || '...'}</span>
        </h2>
        ...
        {(startDate || endDate) && (
          <div className="text-sm text-gray-500 mb-6 bg-gray-50 p-2 rounded border border-gray-200 inline-block">
            ...
            {endDate && (
              <span className="font-medium ml-1">
                au {new Date(endDate).toLocaleDateString()}
              </span>
            )}
          </div>
        )}
        ...
        <div className="space-y-6">
          {learningJourney.skillsHeatmap &&
            learningJourney.skillsHeatmap.length > 0 && (
              <div className="mb-8">
          ...
          <div>
            <h3 className="text-xl font-semibold text-gray-700 mb-3">
              Fiches Consultées ({learningJourney.readFiches?.length ?? 0})
            </h3>
            {learningJourney.readFiches &&
            learningJourney.readFiches.length > 0 ? (
              <ul className="list-disc pl-5 space-y-2">
                {learningJourney.readFiches.map((fiche, index) => {
                  const score = bestScoresByFicheId.get(fiche.ficheId);
                  return (
                    <li
                      key={`${fiche.ficheId}-${index}`}
                      className="text-gray-600"
                    >
                      <Link
                        to={`/memofiche/${fiche.ficheId}`}
                        onClick={onClose}
                      ...
                      {score !== undefined && (
                        <span
                          className={`ml-2 font-semibold ${score >= 80 ? 'text-green-600' : 'text-amber-600'}`}
                        >
                          - Validée à {formatScore(score)}
                        </span>
                      )}
                    </li>
                  );
              ...
              <p className="text-gray-500 italic">
                Aucune fiche lue pour le moment.
              </p>
            )}
          </div>
          ...
          <div>
            ...
            learningJourney.quizHistory.length > 0 ? (
              <ul className="list-disc pl-5 space-y-2">
                {learningJourney.quizHistory.map((quiz, index) => (
                  <li key={index} className="text-gray-600">
                    ...
                    <span
                      className={
                        quiz.score >= 80 ? 'text-green-600' : 'text-amber-600'
                      }
                    >
              ...
              <p className="text-gray-500 italic">
                Aucun quiz effectué pour le moment.
              </p>
            )}
          </div>
          ...
          <div>
            <h3 className="text-xl font-semibold text-gray-700 mb-3">
              Médias Consultés ({learningJourney?.viewedMediaIds?.length ?? 0})
            </h3>
            {learningJourney?.viewedMediaIds &&
            learningJourney.viewedMediaIds.length > 0 ? (
              ...
              <p className="text-gray-500 italic">
                Aucun média consulté pour le moment.
              </p>
            )}
          </div>
        </div>
