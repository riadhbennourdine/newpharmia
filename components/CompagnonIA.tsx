...
const TypewriterMessage = ({
  ...
  const renderContent = (content: string) => {
    return content
      .replace(
        /\*\*(.*?)\*\*/g,
        '<strong class="font-bold text-teal-600">$1</strong>',
      )
      .replace(
        /^\s*[•|*|-]\s+(.*)/gm,
        '<div class="flex items-start gap-2 my-1"><span class="text-teal-500 mt-1.5 text-xs">●</span><span>$1</span></div>',
      )
      .replace(/\n/g, '<br />');
  };
...
const CompagnonIA: React.FC<Props> = ({ user, onClose }) => {
      ...
      const triggerInitialScenario = async () => {
        ...
        try {
          ...
          setMessages([
            {
              role: 'model',
              text: 'Erreur lors du lancement de la simulation.',
            },
          ]);
        } finally {
          setIsLoading(false);
  ...
  const handleEvaluate = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'model', text: "Désolé, je n'ai pas pu générer l'évaluation." },
      ]);
    } finally {
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'model', text: err.message || 'Une erreur est survenue.' },
      ]);
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="flex items-center justify-between p-5 bg-gradient-to-r from-teal-600 to-teal-500 text-white shadow-md z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-teal-50 font-medium tracking-wide uppercase opacity-90">
                Mode Apprenant : {topic || 'Sélection...'}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="p-2.5 hover:bg-white/20 rounded-full transition-all active:scale-90"
            >
        ...
        {!isTopicSelected ? (
          <div className="flex flex-col items-center justify-center flex-grow p-10 text-center space-y-8 bg-slate-50/50">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-500 text-lg">
                Choisissez un sujet pour démarrer la simulation avec votre
                Coach.
              </p>
            </div>
              ...
              >
                <div className="text-left">
                  <p className="text-xs text-teal-600 font-bold uppercase tracking-wider">
                    Continuer l'entretien précédent
                  </p>
                  <p className="text-lg font-bold text-teal-800">
                    {lastSimulation.topic}
                  </p>
                </div>
          ...
          <div className="flex-grow p-8 overflow-y-auto bg-slate-50 animate-fade-in">
            <div className="text-center mb-8">
              <div className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-white mb-4 ring-8 ring-teal-50 shadow-lg relative">
                ...
                <div className="absolute -bottom-2 bg-teal-600 text-white text-xs px-3 py-1 rounded-full font-bold">
                  SCORE
                </div>
              </div>
            ...
            {evaluationResult.recommendedFiches &&
              evaluationResult.recommendedFiches.length > 0 && (
                  ...
                  <div className="grid gap-3">
                    {evaluationResult.recommendedFiches.map((fiche: any) => (
                      <Link
                        ...
                        onClick={onClose}
                      >
                        <span className="font-semibold text-slate-700 group-hover:text-teal-700">
                          {fiche.title}
                        </span>
          ...
          <>
            <div
              ...
              className="flex-grow p-6 space-y-6 overflow-y-auto bg-slate-50/50 scroll-smooth"
            >
              {messages.map((msg, index) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`max-w-[88%] p-4 rounded-2xl shadow-sm relative ${
                        msg.role === 'user'
              ...
              <div className="p-4 bg-white border-t border-slate-100 rounded-b-3xl relative z-20">
                ...
                <div className="text-center mt-2">
                  <p className="text-[10px] text-slate-400 font-medium">
                    Appuyez sur Entrée pour envoyer • Shift + Entrée pour un
                    saut de ligne
                  </p>
                </div>
              </div>
