...
const TeamPlanning: React.FC<Props> = ({ group, onUpdate }) => {
  const [planning, setPlanning] = useState<GroupAssignment[]>(
    group.planning || [],
  );
  const [isPlanningEnabled, setIsPlanningEnabled] = useState(
    group.isPlanningEnabled || false,
  );
  ...
  const handleAddFiche = (hit: any) => {
    ...
    if (planning.some((p) => p.ficheId === hit.objectID)) {
      setMessage({
        type: 'error',
        text: 'Cette fiche est déjà dans le planning.',
      });
      return;
    }
  ...
  const savePlanning = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({
          type: 'success',
          text: 'Planning et mode de parcours enregistrés !',
        });
        onUpdate();
      } else {
    ...
    <div className="bg-white rounded-xl shadow-lg p-6 mb-8 border border-teal-100">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-3 bg-slate-50 p-2 px-4 rounded-full border border-slate-200 shadow-sm">
          <span
            className={`text-sm font-bold ${isPlanningEnabled ? 'text-teal-600' : 'text-slate-500'}`}
          >
          ...
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${isPlanningEnabled ? 'translate-x-6' : 'translate-x-1'}`}
            />
          </button>
        </div>
      </div>
      ...
      {isPlanningEnabled && (
        <div className="mb-6 p-4 bg-teal-50 border border-teal-100 rounded-lg text-sm text-teal-800">
          <strong>Mode Guidé Actif :</strong> Seules les fiches prévues
          ci-dessous seront accessibles par vos préparateurs. Les autres seront
          grisées dans leur bibliothèque.
        </div>
      )}
      ...
      <div className="space-y-4">
        {planning.length === 0 ? (
          <p className="text-center text-slate-500 italic py-4">
            Aucune fiche planifiée.
          </p>
        ) : (
          planning.map((item, idx) => (
            <div
              key={idx}
              className="flex flex-col md:flex-row items-center gap-4 p-4 bg-slate-50 rounded-lg border border-slate-200"
            >
              <div className="flex-1 w-full">
                <p className="font-bold text-slate-800">
                  {ficheDetails[item.ficheId] || 'Chargement...'}
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 w-full md:w-auto">
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChange(idx, 'startDate', new Date(e.target.value))
                    }
