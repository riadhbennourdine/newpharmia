...
interface ManageWebinarResourcesModalProps {
    ...
    onClose: () => void;
    onSave: (webinarId: string, resources: WebinarResource[], linkedMemofiches: (ObjectId | string)[]) => void;
}
...
const ManageWebinarResourcesModal: React.FC<ManageWebinarResourcesModalProps> = ({ webinarId, resources, linkedMemofiches, onClose, onSave }) => {
    ...
    const [localLinkedMemofiches, setLocalLinkedMemofiches] = useState<(ObjectId | string)[]>(linkedMemofiches || []);
        ...
        const fetchLinkedFicheDetails = async () => {
            if (localLinkedMemofiches.length > 0) {
                try {
                    ...
                    if (!response.ok) throw new Error('Failed to fetch linked fiches details');
    ...
    const handleLinkFiche = (fiche: CaseStudy) => {
        if (!localLinkedMemofiches.find(id => id.toString() === fiche._id.toString())) {
            setLocalLinkedMemofiches([...localLinkedMemofiches, fiche._id]);
        }
    };

    const handleUnlinkFiche = (ficheId: ObjectId | string) => {
        setLocalLinkedMemofiches(localLinkedMemofiches.filter(id => id.toString() !== ficheId.toString()));
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] flex flex-col">
                <h3 className="text-xl font-bold mb-4">Gérer les Médias et Mémofiches</h3>
                
                <div className="flex-grow overflow-y-auto pr-2">
                    {/* Section Mémofiches */}
                    <div className="mb-6 border-b pb-6">
                        <h4 className="text-lg font-semibold mb-2 text-slate-700">Mémofiches Liées</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Rechercher une mémofiche</label>
                                <input
                                    type="text"
                                    value={searchTerm}
                                ...
                                <div className="mt-2 h-40 overflow-y-auto border rounded-md">
                                    {isSearching && <div className="p-2 flex justify-center items-center h-full"><Spinner /></div>}
                                    {searchError && <p className="p-2 text-red-500">{searchError}</p>}
                                    {!isSearching && searchResults.length > 0 && (
                                        <ul>
                                            {searchResults.map(fiche => (
                                                <li key={fiche._id.toString()} className="flex justify-between items-center p-2 hover:bg-gray-100">
                                                    <span>{fiche.title}</span>
                                                    <button onClick={() => handleLinkFiche(fiche)} className="text-sm bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600">Lier</button>
                                                </li>
                                            ))}
                                        </ul>
                                    )}
                                    {!isSearching && searchTerm.length > 2 && searchResults.length === 0 && <p className="p-2 text-gray-500">Aucun résultat</p>}
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Mémofiches actuellement liées</label>
                                <div className="mt-2 h-40 overflow-y-auto border rounded-md p-2 space-y-2">
                                    {linkedFichesDetails.length > 0 ? (
                                        linkedFichesDetails.map(fiche => (
                                            <div key={fiche._id.toString()} className="flex justify-between items-center bg-gray-100 p-2 rounded">
                                                <span className="text-sm">{fiche.title}</span>
                                                <button onClick={() => handleUnlinkFiche(fiche._id)} className="text-xs text-red-500 hover:text-red-700">Détacher</button>
                                            </div>
                                        ))
                                    ) : <p className="text-gray-500 text-sm">Aucune mémofiche liée.</p>}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Section Médias */}
                    <div>
                        <h4 className="text-lg font-semibold mb-2 text-slate-700">Autres Médias</h4>
