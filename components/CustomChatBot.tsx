...
const CustomChatBot: React.FC<{ context: string; title: string }> = ({
  context,
  title,
}) => {
  ...
  const renderChatMessage = (text: string) => {
    const rendered = text
      ...
      .replace(
        /^### (.*$)/gim,
        '<h4 class="text-sm font-bold text-slate-800 mt-2 mb-1 uppercase tracking-tight">$1</h4>',
      )
      // Handle Bullet Points (* item or - item)
      .replace(
        /^\s*[*|-]\s+(.*$)/gim,
        '<div class="flex items-start gap-1.5 ml-1 my-0.5"><span class="text-teal-500 mt-1">•</span><span class="text-sm">$1</span></div>',
      );
  ...
  return (
    <div className="flex flex-col max-h-[600px] bg-white rounded-lg shadow-lg border border-slate-200/80">
            ...
            >
              <p
                ...
                dangerouslySetInnerHTML={{
                  __html: renderChatMessage(msg.text),
                }}
              />
      ...
      <form
        ...
        className="p-4 border-t border-slate-200/80 bg-slate-50 rounded-b-lg"
      >
        <div className="flex items-center space-x-2">
          ...
          >
            <img
              src="https://pharmaconseilbmb.com/photos/site/bot.gif"
              alt="Envoyer"
              className="h-12 w-12"
            />
          </button>
