        ...
        <div className="p-6 overflow-y-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-lg font-semibold text-teal-700 mb-3">
                Pharmaciens ({group.pharmacistNames?.length || 0})
              </h3>
            ...
            <div>
              <h3 className="text-lg font-semibold text-teal-700 mb-3">
                Préparateurs ({group.preparatorIds?.length || 0})
              </h3>
          ...
          <div className="mt-6 border-t pt-4">
            ...
            <p className="text-sm text-gray-600">
              ...
              {group.managedBy
                ? (group as any).managedByName || 'Admin/Formateur'
                : 'Non assigné'}
            </p>
            <p className="text-sm text-gray-600">
              <strong>Date d'expiration :</strong>{' '}
              {group.subscriptionEndDate
                ? new Date(group.subscriptionEndDate).toLocaleDateString()
                : 'N/A'}
            </p>
          </div>
        </div>
...
const PreparatorItem: React.FC<{
  preparatorId: string;
  onSelect: (prep: { id: string; name: string }) => void;
  startDate?: string;
  endDate?: string;
}> = ({ preparatorId, onSelect, startDate, endDate }) => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const learningJourneyResponse = await fetch(
          `/api/users/${preparatorId}/learning-journey?${params.toString()}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
