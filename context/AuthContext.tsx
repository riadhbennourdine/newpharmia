...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const markFicheAsRead = useCallback(
    async (ficheId: string) => {
      if (
        !user ||
        !token ||
        user.readFiches?.some((f) => f.ficheId === ficheId)
      ) {
        return;
      }
  ...
  const saveQuizResult = useCallback(
    async (result: { quizId: string; score: number; completedAt: Date }) => {
