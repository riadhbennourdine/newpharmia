...
const ReadFichesPage: React.FC = () => {
  ...
  const [readFiches, setReadFiches] = useState<
    (CaseStudy & { readAt: Date })[]
  >([]);
  ...
  useEffect(() => {
    const fetchReadFiches = async () => {
      ...
      try {
        ...
        const readFichesResponse = await fetch(
          `/api/users/${userId}/read-fiches`,
        );
        ...
        if (userReadFiches && userReadFiches.length > 0) {
          ...
          const fichesPromises = userReadFiches.map(
            async (readInfo: { ficheId: string; readAt: Date }) => {
              try {
                ...
                const ficheResponse = await fetch(
                  `/api/memofiches/${readInfo.ficheId}`,
                  { headers },
                );
                if (ficheResponse.ok) {
          ...
          setReadFiches(
            fichesDetails.filter(Boolean) as (CaseStudy & { readAt: Date })[],
      ...
      {readFiches.length === 0 ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {readFiches.map((fiche) => {
            const quizResult = currentUser?.quizHistory?.find(
              (q) => q.quizId === fiche._id,
            );
                ...
                <Link to={`/memofiche/${fiche._id}`} className="flex-grow">
                  <img
                    src={getAbsoluteImageUrl(
                      fiche.coverImageUrl ||
                        'https://via.placeholder.com/300x200?text=MemoFiche',
                    )}
                    alt={fiche.title}
