...
import {
  ...
  TAX_RATES,
  CROPT_BANK_DETAILS,
  SKILL_SEED_BANK_DETAILS,
  WEBINAR_PRICE,
  PHARMIA_WEBINAR_PRICE_HT,
  PHARMACONSEIL_BANK_DETAILS,
} from '../constants';
...
const CheckoutPage: React.FC = () => {
  ...
  const hasCropTunis = webinarsInOrder.some(
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        const webinarIds = orderData.items
          .filter(
            (item) =>
              (!item.type || item.type === ProductType.WEBINAR) &&
              item.webinarId,
          )
  ...
  const handleSubmitPayment = async (e: React.FormEvent) => {
    ...
    if (!selectedFile || !orderId || !token) {
      alert(
        'Veuillez sélectionner un fichier et vous assurer que vous êtes connecté.',
      );
      return;
    }
    ...
    try {
      ...
      if (!uploadResponse.ok) {
        ...
        throw new Error(
          `Échec du téléversement du fichier. Le serveur a répondu avec le statut : ${uploadResponse.status}`,
        );
      }
  ...
  const handleKonnectPayment = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errData.message || "Erreur lors de l'initialisation du paiement",
        );
      }
      ...
      <ul className="mb-4 space-y-2 text-sm text-slate-600">
        {webinarsInOrder.map((w) => {
          ...
          if (w.group === WebinarGroup.MASTER_CLASS) {
            itemDisplayPrice =
              webinarBasePrice * (1 + TAX_RATES.TVA) + TAX_RATES.TIMBRE;
            ...
            // Fallback for other webinar types, if any
            itemDisplayPrice =
              webinarBasePrice * (1 + TAX_RATES.TVA) + TAX_RATES.TIMBRE;
          }
          return (
            <li key={w._id.toString()} className="flex justify-between">
              ...
              <span>
                {itemDisplayPrice.toFixed(3)} TND{' '}
        ...
        {order.items
          .filter((i) => i.type === ProductType.PACK || !!i.packId)
          .map((item, idx) => {
            const masterClassPack = MASTER_CLASS_PACKS.find(
              (p) => p.id === item.packId,
            );
            const pharmiaCreditPack = PHARMIA_CREDIT_PACKS.find(
              (p) => p.id === item.packId,
            );
  ...
  const hasMasterClassPacks = order?.items.some(
    (i) =>
      (i.type === ProductType.PACK &&
        i.packId &&
        MASTER_CLASS_PACKS.some((p) => p.id === i.packId)) ||
      (i.productId && i.productId.startsWith('MC')),
  );
  const hasPharmIAPacks = order?.items.some(
    (i) =>
      i.type === ProductType.PACK &&
      i.packId &&
      PHARMIA_CREDIT_PACKS.some((p) => p.id === i.packId),
  );
  const hasMasterClassWebinars = webinarsInOrder.some(
    (w) => w.group === WebinarGroup.MASTER_CLASS,
  );
  const hasPharmIAWebinars = webinarsInOrder.some(
  ...
  const isPdfRib = bankDetails.imageUrl
    ? bankDetails.imageUrl.toLowerCase().includes('.pdf')
    : false;

  // Determine if Konnect (card) payment should be available (only for MasterClass items)
  const containsMasterClassItems =
    hasMasterClassPacks || hasMasterClassWebinars;
            ...
            <div className="space-y-2 text-sm text-slate-600">
              {(() => {
                ...
                order.items.forEach((item) => {
                  const isPack =
                    item.type === ProductType.PACK || !!item.packId;

                  if (isPack && item.packId) {
                    const pack = MASTER_CLASS_PACKS.find(
                      (p) => p.id === item.packId,
                    );
                    if (pack) {
                    ...
                    // Assume it's a webinar
                    const webinarDetails = webinarsInOrder.find(
                      (w) => w._id === (item.webinarId || item.productId),
                    );
                    if (webinarDetails) {
                      if (webinarDetails.group === WebinarGroup.MASTER_CLASS) {
                        ...
                        webinarDetails.group === WebinarGroup.CROP_TUNIS
                      ) {
                        cropWebinarsTTC += webinarDetails.price || 80.0; // CROP prices are considered TTC (e.g. 80.000 DT)
                      } else if (
                        webinarDetails.group === WebinarGroup.PHARMIA
                      ) {
                        const webinarBasePrice =
                          webinarDetails.price || PHARMIA_WEBINAR_PRICE_HT;
                        calculatedTotalHT += webinarBasePrice;
                        calculatedTotalTVA += webinarBasePrice * TAX_RATES.TVA;
                      }
                    }
                ...
                const hasTaxableItems =
                  calculatedTotalHT > 0 || calculatedTotalTVA > 0;
                const finalCalculatedTTC =
                  calculatedTotalHT +
                  calculatedTotalTVA +
                  (hasTaxableItems ? calculatedStampDuty : 0) +
                  cropWebinarsTTC;
                ...
                {(() => {
                  ...
                  order.items.forEach((item) => {
                    ...
                    if (isPack && item.packId) {
                      ...
                      if (webinarDetails) {
                        if (
                          webinarDetails.group === WebinarGroup.MASTER_CLASS
                        ) {
                          const webinarBasePrice = webinarDetails.price || 0; // MC prices are HT
                          calculatedTotalHT += webinarBasePrice;
                          calculatedTotalTVA +=
                            webinarBasePrice * TAX_RATES.TVA;
                        } else if (
                          webinarDetails.group === WebinarGroup.CROP_TUNIS
                        ) {
                          cropWebinarsTTC += webinarDetails.price || 80.0; // CROP prices are considered TTC
                        } else if (
                          webinarDetails.group === WebinarGroup.PHARMIA
                        ) {
                          const webinarBasePrice =
            ...
            <div className="mt-8">
              ...
              <div className={`grid gap-6 grid-cols-1 md:grid-cols-2`}>
                ...
                >
                  ...
                  <p className="text-sm text-slate-500 text-center mt-2">
                    {containsMasterClassItems
                      ...
                      : 'Paiement sécurisé via GPG'}
                  </p>
                  <span className="mt-4 px-4 py-2 bg-teal-600 text-white text-sm font-bold rounded-lg group-hover:bg-teal-700">
                    Choisir
                  </span>
                </button>
                ...
                >
                  ...
                  <span className="mt-4 px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-lg group-hover:bg-blue-700">
                    Choisir
                  </span>
                </button>
              </div>
                ...
                <div>
                  <div className="mb-6">
                    ...
                    <p className="text-sm text-slate-600">
                      Veuillez effectuer un virement bancaire du montant total
                      sur le compte suivant, en indiquant votre numéro de
                      commande dans le libellé de la transaction.
                    </p>
                    <div className="mt-4 p-4 bg-slate-50 rounded-md text-sm border border-slate-200">
                      <p className="mb-1">
                        <strong>Bénéficiaire:</strong> {bankDetails.holder}
                      </p>
                      <p className="mb-1">
                        <strong>Banque:</strong> {bankDetails.bank} (
                        {bankDetails.branch})
                      </p>
                      <p className="mb-3">
                        <strong>RIB:</strong>{' '}
                        <span className="font-mono text-base bg-slate-100 px-2 py-1 rounded">
                          {bankDetails.rib}
                        </span>
                      </p>
                      ...
                      {isPdfRib ? (
                        ...
                        >
                          <img
                            ...
                            alt="RIB"
                            className="rounded-md shadow-sm w-full max-w-xs"
                          />
                        </a>
                      )}
                    </div>
                  ...
                  <form onSubmit={handleSubmitPayment}>
                    ...
                    <p className="text-sm text-slate-600 mb-4">
                      Une fois le virement effectué, veuillez téléverser une
                      capture d'écran ou un reçu comme preuve.
                    </p>
