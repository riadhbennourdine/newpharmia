...
const GeneratorView: React.FC = () => {
   ...
   const [memoFicheType, setMemoFicheType] = useState<'maladie' | 'pharmacologie' | 'dermocosmetique' | 'exhaustive' | 'dispositifs-medicaux' |
       'ordonnances' | 'communication' | 'micronutrition'>('maladie');
 ...
 - **recommendations**: Cet objet doit contenir quatre listes :
   ...
   - **associatedProducts**: Liste des produits associés. Chaque élément doit être une chaîne de caractères. Ne pas dépasser 12 points. Chaque point doit
     concerner un produit ou une classe de produits et commencer par le nom en évidence (ex: **Paracétamol**).
 ...
 - Interdiction formelle d'utiliser les sections suivantes, car elles ne sont absolument pas pertinentes pour la pharmacologie ou les dispositifs médicaux : "Aperçu pathologie", "Signaux d'alerte", "Produits associés", "Etat et besoin de la peau", "Conseiller une consultation dermatologique", "Produit
   principal", "Hygiène de vie", "Conseils alimentaires".
 ...
 - Le contenu de chaque section doit être une liste à puces, où chaque ligne commence par un mot-clé pertinent mis en évidence avec des doubles astérisques
   (par exemple, **Mot-clé**).
 `;
 ...
 `;
     ...
     if (memoFicheType === 'maladie') {
         prompt = `Génère une mémofiche pour des professionnels de la pharmacie sur le sujet : "${sourceText}". Le thème pédagogique est "${selectedTheme}"
     et le système clinique est "${selectedSystem}".${formattingInstructions}`;
     } else if (memoFicheType === 'pharmacologie') {
         prompt = `Génère une mémofiche de pharmacologie sur le principe actif ou la classe : "${sourceText}". Le thème de la mémofiche est "${pharmaTheme}
     " et la pathologie cible est "${pharmaPathology}".${pharmacologieFormattingInstructions}`;
     } else if (memoFicheType === 'dispositifs-medicaux') {
         prompt = `Génère une mémofiche sur le dispositif médical : "${sourceText}". Le thème de la mémofiche est "${pharmaTheme}" et l'indication
     principale est "${pharmaPathology}".${dispositifsMedicauxFormattingInstructions}`;
     } else if (memoFicheType === 'dermocosmetique') {
         prompt = `Vous devez impérativement utiliser le modèle de mémofiche de dermocosmétique. Ne pas utiliser le modèle de maladies courantes. Génère
     une mémofiche de dermocosmetique sur le sujet : "${sourceText}". Le thème pédagogique est "${selectedTheme}" et le système clinique est "${selectedSystem}
     ".${formattingInstructions}`;
     } else if (memoFicheType === 'communication') {
         prompt = `En tant qu'expert en communication pharmaceutique, analyse le texte suivant et génère une mémofiche de type 'communication'. La
     mémofiche doit inclure un titre pertinent, une courte description, un résumé d'introduction, une section 'cas comptoir' (patientSituation) et plusieurs
     sections personnalisées (customSections) qui décomposent le sujet de manière logique et facile à comprendre pour un professionnel de la pharmacie. Le
     contenu de chaque section doit être détaillé, professionnel et rédigé dans un style clair et concis. Chaque section doit avoir un titre et un contenu. Le
     contenu de chaque section doit être une liste à puces. Chaque point de la liste doit être sur une nouvelle ligne (en utilisant '\\n'). Chaque ligne doit
     commencer par un mot-clé pertinent mis en évidence avec des doubles astérisques (par exemple, **Mot-clé**). Le texte à analyser est :\n\n${sourceText}`;
     } else if (memoFicheType === 'ordonnances') {
         prompt = `Génère une mémofiche sur l'analyse d'une ordonnance pour le sujet : "${sourceText}". Le thème pédagogique est "${selectedTheme}" et le
     système clinique est "${selectedSystem}".\nTu dois générer un objet JSON avec les clés suivantes : "ordonnance", "analyseOrdonnance",
     "conseilsTraitement", "informationsMaladie", "conseilsHygieneDeVie", "conseilsAlimentaires", "ventesAdditionnelles", "references".\nLe contenu de chaque
     clé doit être un tableau de chaînes de caractères.\nChaque chaîne de caractères doit correspondre à un point de la section.\n\nVoici le détail de chaque
     section :\n- **ordonnance**: Ordonnance. Contenant les détails du patient, la pathologie, la prescription et la durée.\n- **analyseOrdonnance**: Analyse
     de l'ordonnance. Contenant l'analyse de la prescription, la vérification essentielle et le profil du patient.\n- **conseilsTraitement**: Conseils sur le
     traitement médicamenteux. Pour chaque médicament de la prescription, créer un objet avec les clés "medicament" et "conseils". "medicament" est le nom du
     médicament. "conseils" est un tableau de chaînes de caractères contenant les conseils d'administration, les effets indésirables éventuels et les
     précautions d'emploi.\n- **informationsMaladie**: Informations sur la maladie. Ne pas commencer par le mot "Mécanisme". Commencer par le nom de la maladie
     en gras (par exemple, **Rhinite allergique**).\n- **conseilsHygieneDeVie**: Conseils hygiène de vie. Ne doit contenir que des Conseils hygiène de vie.\n-
     **conseilsAlimentaires**: Conseils alimentaires. Indiquer les aliments à consommer à privilégier, et les aliments à éviter. Ne pas lister de
     micronutriments.\n- **ventesAdditionnelles**: Ventes additionnelles. Répartir les produits dans les sous-rubriques suivantes (si pertinentes) :
     "complementsAlimentaires", "accessoires", "dispositifs", "cosmetiques". Pour chaque produit, inclure les doses, les posologies et les modes
     d'administration.\n\n${formattingInstructions}\n`;
     } else if (memoFicheType === 'micronutrition') {
         prompt = `Créer une mémofiche claire et concise pour initier les pharmaciens d'officine et leurs collaborateurs débutants à la micronutrition et
     son apport pour les patients souffrant de [maladie]. La mémofiche doit se concentrer sur le déclenchement de la discussion au comptoir et comment créer le
     besoin, puis explication de la [maladie] , les traitements conventionnels. Puis l'approche micronutritionnelle en expliquant simplement les mécanismes
     d'action de chaque micronutriment et en proposant des conseils alimentaires et des règles d'hygiène de vie. utiliser un langage accessible et
     encourageant. Le thème pédagogique est "${selectedTheme}" et le système clinique est "${selectedSystem}". Le sujet détaillé est : "${sourceText}".`;
     } else { // exhaustive
       ...
       <div className="animate-fade-in">
         ...
         <div className="text-center mt-8 flex justify-center items-center space-x-4">
             ...
             <button
                 ...
                 className="text-lg bg-teal-600 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-teal-700 hover:shadow-lg transform
     hover:-translate-y-0.5 transition-all duration-300 disabled:bg-slate-400"
                 disabled={isGeneratingTools}
             >
               {isGeneratingTools ? (
                 <>
   ...
   return (
     <div className="max-w-4xl mx-auto animate-fade-in p-4 md:p-8">
       <button onClick={() => navigate('/dashboard')} className="flex items-center text-sm font-medium text-teal-600 hover:text-teal-800 mb-6
     transition-colors">
     ...
     disabled={isLoading}>
                 ...
                 {youtubeLinks.length < 3 && (
                     <button type="button" onClick={addYoutubeLink} className="flex items-center px-3 py-1 bg-teal-100 text-teal-800 text-sm font-semibold
     rounded-md hover:bg-teal-200 mt-2" disabled={isLoading}>
       ...
       <div className="mt-6 text-center">
         <button
           ...
           disabled={isLoading || !sourceText.trim() || (memoFicheType === 'maladie' && (!selectedTheme || !selectedSystem)) || (memoFicheType ===
     'pharmacologie' && (!pharmaTheme.trim() || !pharmaPathology.trim())) || (memoFicheType === 'dermocosmetique' && (!selectedTheme || !selectedSystem)) ||
     (memoFicheType === 'micronutrition' && (!selectedTheme || !selectedSystem))}
           className={`text-xl font-bold py-3 px-12 rounded-lg shadow-xl hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300 w-full md:w-auto
           ${isLoading ? 'bg-slate-400 text-white cursor-not-allowed' : 'bg-teal-600 text-white hover:bg-teal-700'}`}
         >
           {isLoading ? (
             <>
               <Spinner className="-ml-1 mr-3 h-6 w-6 text-white" />
               <span>Génération en cours...</span>
             </>
           ) : (
             <>
               <SparklesIcon className="inline-block h-6 w-6 mr-2" />
         ...
         <button
           ...
           disabled={isLoading}
           className="mt-4 block w-full md:inline-block md:w-auto text-sm text-slate-500 hover:text-slate-700 transition-colors"
         >
           Ou Sauvegarder les données brutes manuellement
         </button>
       </div>
     </div>
   );
