...
const GeneratorView: React.FC = () => {
  ...
  const [memoFicheType, setMemoFicheType] = useState<
    | 'maladie'
    | 'pharmacologie'
    | 'dermocosmetique'
  ...
  const handleGenerate = async () => {
    ...
    if (
      ...
      (!sourceText.trim() || !selectedTheme || !selectedSystem)
    )
      return;
    if (
      (memoFicheType === 'pharmacologie' ||
        memoFicheType === 'dispositifs-medicaux') &&
      (!sourceText.trim() || !pharmaTheme.trim() || !pharmaPathology.trim())
    )
      return;
    if (
    ...
    if (memoFicheType === 'le-medicament') {
      try {
        if (!sourceText.trim()) {
          setError(
            'Le sujet détaillé (titre du médicament) est requis pour la création manuelle.',
          );
          setIsLoading(false);
          return;
        }
        const newMemoFiche: CaseStudy = {
          ...
          title:
            sourceText.substring(0, 50) + (sourceText.length > 50 ? '...' : ''), // Use first part of sourceText as title
          shortDescription:
        ...
        alert(
          "Mémofiche créée manuellement avec succès ! Vous pouvez maintenant la modifier et utiliser l'IA pour générer du contenu.",
        );
        navigate(`/edit-memofiche/${savedCase._id}`); // Navigate directly to edit page
      } catch (err: any) {
        console.error('Error creating manual memo fiche:', err);
        setError(
          err.message || 'La création de la mémofiche manuelle a échoué.',
        );
      } finally {
        setIsLoading(false);
      }
    ...
    try {
      ...
      setError(
        err.message ||
          "La génération de la mémofiche a échoué. Vérifiez les données d'entrée et réessayez.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleValidateAndGenerateTools = async () => {
    ...
    try {
      ...
      if (savedCase && savedCase._id) {
        ...
        alert(
          'Mémofiche générée et sauvegardée avec succès ! Vous la trouverez dans votre tableau de bord.',
        );
        navigate('/dashboard');
      }
    } catch (err: any) {
      setError(
        err.message || 'La génération des outils pédagogiques a échoué.',
      );
  ...
  const handleSaveManually = async () => {
    if (!sourceText.trim()) {
      setError(
        'Le sujet détaillé de la mémofiche est requis pour la sauvegarde manuelle.',
      );
      return;
    }
    ...
    try {
      ...
      setError(
        err.message || 'La sauvegarde manuelle de la mémofiche a échoué.',
      );
    } finally {
      setIsLoading(false);
    }
  ...
  if (generatedCase) {
    return (
      <div className="animate-fade-in">
        ...
        {error && (
          <div
            className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-md relative mb-6 max-w-4xl mx-auto"
            role="alert"
          >
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in p-4 md:p-8">
      ...
      <button
        ...
        className="flex items-center text-sm font-medium text-teal-600 hover:text-teal-800 mb-6 transition-colors"
      >
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-slate-600">
          {step === 1
            ? 'Choisissez un type de mémofiche pour commencer.'
            : 'Remplissez les détails pour générer votre mémofiche.'}
        </p>
      </div>
      ...
      {step === 1 ? (
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          {(memoFicheType === 'maladie' ||
            memoFicheType === 'dermocosmetique' ||
            memoFicheType === 'ordonnances' ||
            memoFicheType === 'communication' ||
            memoFicheType === 'micronutrition' ||
            memoFicheType === 'savoir') && (
          ...
          {(memoFicheType === 'pharmacologie' ||
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                <label
                  htmlFor="pharma-pathology-select"
                  className="block text-lg font-medium text-slate-700 mb-2"
                >
          ...
          {memoFicheType === 'le-medicament' && (
            <div className="space-y-6 mb-6 p-4 border border-slate-200 rounded-md bg-slate-50">
              ...
              <div>
                <label
                  htmlFor="theme-select-le-medicament"
                  className="block text-lg font-medium text-slate-700 mb-2"
                >
                  Thème Pédagogique
                </label>
              ...
              <h3 className="text-xl font-bold text-slate-800">
                Contenu additionnel pour "Le médicament"
              </h3>
              <div>
                <label
                  htmlFor="youtube-explainer-url"
                  className="block text-lg font-medium text-slate-700 mb-2"
                >
              ...
              {youtubeLinks.map((link, index) => (
                <div key={index} className="flex items-center space-x-2 mb-2">
                  <input
                    ...
                    value={link.title}
                    onChange={(e) =>
                      handleYoutubeLinkChange(index, 'title', e.target.value)
                    }
                    className="w-full border-gray-300 rounded-md shadow-sm focus:ring-teal-500 focus:border-teal-500 text-base"
                    disabled={isLoading}
                  />
                  <input
                    ...
                    value={link.url}
                    onChange={(e) =>
                      handleYoutubeLinkChange(index, 'url', e.target.value)
                    }
                    className="w-full border-gray-300 rounded-md shadow-sm focus:ring-teal-500 focus:border-teal-500 text-base"
                    disabled={isLoading}
                  />
          ...
          <div className="mt-6 text-center">
            ...
            {(() => {
              ...
              if (
                ...
                isDisabled =
                  isDisabled || !pharmaTheme.trim() || !pharmaPathology.trim();
              }
