...
const MemoFicheCard: React.FC<{
  ...
  const handleDelete = (e: React.MouseEvent) => {
    ...
    if (
      window.confirm(
        `Êtes-vous sûr de vouloir supprimer la mémofiche "${caseStudy.title}" ?`,
      )
    ) {
  ...
  const handleNavigate = () => {
    if (canAccess) {
      ...
      alert(
        "Cette fiche n'est pas encore ouverte dans votre parcours d'apprentissage par votre pharmacien.",
      );
    }
  };
  ...
  return (
    <div
      className={`group bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden flex flex-col ${!canAccess ? 'opacity-75 grayscale-[0.5]' : ''}`}
    >
          ...
          {!canAccess && (
            <div className="absolute inset-0 bg-slate-900/60 flex flex-col items-center justify-center p-4 text-center">
              ...
              <p className="text-white text-xs font-bold uppercase tracking-widest">
                {caseStudy.isLocked
                  ? 'Premium'
                  : 'Verrouillé par le Pharmacien'}
              </p>
            </div>
          )}
        </div>
        <div className="p-4 flex-grow flex flex-col">
          <h3 className="text-lg font-bold text-slate-800 group-hover:text-teal-700 truncate">
            {caseStudy.title}
          </h3>
          <p className="text-xs font-semibold text-teal-600 uppercase tracking-wide mt-1">
            {caseStudy.theme} &bull; {caseStudy.system}
          </p>
          <p className="text-xs text-slate-500 mt-1">
            Créé le{' '}
            {new Date(caseStudy.creationDate).toLocaleDateString('fr-FR')}
          </p>
          {(isAdmin || isFormateur) && (
            <p className="text-xs text-slate-500 mt-1">
              Statut: {caseStudy.status}
            </p>
          )}
          <p className="mt-2 text-sm text-slate-600 line-clamp-2 flex-grow">
...
const MemoFichesPage: React.FC = () => {
  ...
  const [selectedFiche, setSelectedFiche] = useState<CaseStudy | undefined>(
    undefined,
  );

  // Planning restriction state
  const [allowedFicheIds, setAllowedFicheIds] = useState<Set<string>>(
    new Set(),
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">
            Toutes les mémofiches
          </h1>
          <p className="text-lg text-slate-600 mt-1">
            Explorez, recherchez et filtrez notre bibliothèque de contenu.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm mb-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          >
            ...
            {TOPIC_CATEGORIES[0].topics.map((t) => (
          ...
          >
            ...
            {TOPIC_CATEGORIES[1].topics.map((t) => (
              <option key={t} value={t}>
                {t}
              </option>
            ))}
