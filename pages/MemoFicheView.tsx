...
import {
  ...
  SparklesIcon,
  SimulationIcon,
} from '../components/Icons'; // Added ImageIcon, BookOpenIcon, SparklesIcon, SimulationIcon
...
const ComparisonCard: React.FC<{ title: string; description: string }> = ({
  title,
  description,
}) => {
    ...
    const fetchLinkedFiche = async () => {
      try {
        ...
        const response = await fetch(
          `/api/memofiches?search=${encodeURIComponent(title)}&limit=1`,
        );
  ...
  const lesionImage =
    linkedFiche && Array.isArray(linkedFiche.patientSituation?.content)
      ? linkedFiche.patientSituation.content.find(
          (c: any) => c.type === 'image',
        )?.value
    ...
    <div className="mb-2 bg-white rounded-lg shadow-sm border border-slate-200/80 overflow-hidden transition-all duration-300">
      ...
      >
        <div className="flex items-center">
          {icon}
          <h3
            className={`text-lg font-bold ${isAlert ? 'text-red-600' : 'text-slate-800'}`}
          >
            {title}
          </h3>
        </div>
      ...
      >
        <div
          className={`p-4 pt-0 pl-4 sm:pl-12 space-y-2 ${isAlert ? 'text-red-600' : 'text-slate-700'} ${contentClassName}`}
        >
            ...
            dangerouslySetInnerHTML={{
              __html:
                typeof item === 'string'
                  ? item.replace(
                      /\*\*(.*?)\*\*/g,
                      `<span class="${keywordClass}">$1</span>`,
                    )
                  : item,
            }}
          ></li>
  ...
  const sections = [
    {
      ...
      content: renderList(ventes.complementsAlimentaires),
    },
        ...
        (section) =>
          section.content && (
            <AccordionSection
              ...
              title={section.title}
              icon={
                <div className="flex items-center justify-center h-6 w-6 mr-3 bg-teal-600 text-white rounded-full font-bold text-sm">
                  +
                </div>
              }
            >
...
const ConseilsTraitementSection: React.FC<{ conseils: any }> = ({
  conseils,
}) => {
  const renderList = (items: string[] | undefined) => {
    if (!items || items.length === 0) return null;
  ...
  return (
    <div>
      {((conseils as { medicament: string; conseils: string[] }[]) || []).map(
        (item, index) => (
          <AccordionSection
            key={index}
...
const ConseilsAlimentairesSection: React.FC<{ conseils: string[] }> = ({
  conseils,
}) => {
  const aPrivilegier = conseils.filter((c) =>
    c.toLowerCase().includes('privilégier'),
  );
    ...
    <div>
      ...
      {aEviter.length > 0 && (
        <AccordionSection
          ...
          icon={
            <div className="flex items-center justify-center h-6 w-6 mr-3 bg-red-600 text-white rounded-full font-bold text-sm">
              -
            </div>
          }
        >
...
export const DetailedMemoFicheView: React.FC<DetailedMemoFicheViewProps> = ({
  caseStudy,
  onBack,
  onStartQuiz,
  onEdit,
  ...
  const premiumCampaign = useMemo(() => {
    ...
    const searchableText = [
      caseStudy.title,
      typeof caseStudy.pathologyOverview === 'string'
  ...
  const handleDelete = () => {
    if (
      window.confirm(
        'Êtes-vous sûr de vouloir supprimer cette mémofiche ? Cette action est irréversible.',
      )
    ) {
      onDelete?.();
    }
  ...
  const getYoutubeEmbedUrl = (url: string | undefined): string | null => {
    ...
    try {
      ...
      if (urlObj.hostname === 'youtu.be') videoId = urlObj.pathname.slice(1);
        ...
        urlObj.hostname.includes('youtubeeducation.com')
      )
  ...
  const formattedDate = new Date(caseStudy.creationDate).toLocaleDateString(
    'fr-FR',
    { day: 'numeric', month: 'long', year: 'numeric' },
  );

  // Helper to check if content is MemoFicheSectionContent[]
  const isMemoFicheSectionContentArray = (
    content: any,
  ): content is MemoFicheSectionContent[] => {
    return (
    ...
    if (isLongContent && hasNumericalHeadings) {
      ...
      if (subsections.length === 0 || !isLongContent) {
        return (
          <div
            dangerouslySetInnerHTML={{
              __html: renderMarkdown(textContent, isRedKeywordSection),
            }}
          />
        );
      }
        ...
        <div>
          {subsections.map((subsection, index) => (
            <AccordionSection
              ...
              icon={
                <div className="flex items-center justify-center h-6 w-6 mr-3 bg-gray-600 text-white rounded-full font-bold text-sm">
                  {index + 1}
                </div>
              }
              startOpen={index === 0}
        ...
        {lines.map((line, index) => {
          ...
          const renderedLine = (
            <div
              key={`line-${index}`}
              dangerouslySetInnerHTML={{
                __html: renderMarkdown(line, isRedKeywordSection),
              }}
            />
          );
  ...
  const renderMarkdown = (
    ...
    if (html.includes('COMPARAISON :')) {
      const regex =
        /COMPARAISON\s*:\s*(.*?)\s*\|\s*DESCRIPTION\s*:\s*(.*?)\s*\|\s*LIEN_REQUIS\s*:\s*true/g;
      return (
    ...
    // Keywords (bold)
    const keywordClass = isRedKeywordSection
    ...
    html = html
      ...
      .map((line) => {
        ...
        if (
          trimmedLine &&
          !trimmedLine.match(/<(h[1-6]|ul|ol|li|div|p|img|iframe)>/i)
        ) {
  ...
  const isMemoFicheSectionContentEmpty = (sectionContent: any): boolean => {
    ...
    return sectionContent.every(
      (item) => !item || !item.value || item.value.trim() === '',
    );
  };
  ...
  const memoContent = useMemo(() => {
    if (caseStudy.type === 'savoir') {
      ...
      const memoSections = (caseStudy.memoSections || [])
        ...
        .map((section, index) => ({
          ...
          title: section.title,
          icon: (
            <div className="flex items-center justify-center h-6 w-6 mr-3 bg-purple-600 text-white rounded-full font-bold text-sm">
              {index + 1}
            </div>
          ),
      ...
      const customSections = (caseStudy.customSections || [])
        ...
        .map((section, index) => ({
          ...
          title: section.title,
          icon: (
            <div className="flex items-center justify-center h-6 w-6 mr-3 bg-blue-600 text-white rounded-full font-bold text-sm">
              {index + 1}
            </div>
          ),
          data: section.content,
      ...
      if (
        !sectionsToDisplay.some((s) => s.id === 'references') &&
        ...
        sectionsToDisplay.push({
          ...
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('references'))}
      ...
      if (effectiveSectionOrder.length > 0) {
        ...
        const remaining = sectionsToDisplay.filter(
          (s) => !effectiveSectionOrder.includes(s.id),
        );
      ...
      const content = [
        {
          ...
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('ordonnance'))}
              className="h-6 w-6 mr-3"
              alt="Ordonnance"
            />
          ),
          content: renderContentWithKeywords(caseStudy.ordonnance),
          startOpen: true,
        },
        {
          ...
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('analyseOrdonnance'))}
              className="h-6 w-6 mr-3"
              alt="Analyse"
            />
          ),
          content: renderContentWithKeywords(caseStudy.analyseOrdonnance),
          startOpen: false,
        },
        {
          id: 'conseilsTraitement',
          title: 'Conseils sur le traitement',
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('conseilsTraitement'))}
              className="h-6 w-6 mr-3"
              alt="Conseils"
            />
          ),
          content: (
            <ConseilsTraitementSection
              conseils={caseStudy.conseilsTraitement}
            />
          ),
          startOpen: false,
        },
        {
          ...
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('informationsMaladie'))}
              className="h-6 w-6 mr-3"
              alt="Maladie"
            />
          ),
          content: renderContentWithKeywords(caseStudy.informationsMaladie),
          startOpen: false,
        },
        {
          id: 'conseilsHygieneDeVie',
          title: "Conseils d'hygiène de vie",
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('conseilsHygieneDeVie'))}
              className="h-6 w-6 mr-3"
              alt="Hygiène de vie"
            />
          ),
          content: renderContentWithKeywords(caseStudy.conseilsHygieneDeVie),
          startOpen: false,
        },
        {
          ...
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('conseilsAlimentaires'))}
              className="h-6 w-6 mr-3"
              alt="Alimentation"
            />
          ),
          content: (
            <ConseilsAlimentairesSection
              conseils={caseStudy.conseilsAlimentaires as string[]}
            />
          ),
          startOpen: false,
        },
        {
          ...
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('ventesAdditionnelles'))}
              className="h-6 w-6 mr-3"
              alt="Ventes"
            />
          ),
          content: (
            <VentesAdditionnellesSection
              ventes={caseStudy.ventesAdditionnelles}
            />
          ),
          startOpen: false,
        },
        {
          ...
          content: renderContentWithKeywords(caseStudy.references),
          contentClassName: 'text-sm',
          startOpen: false,
        },
      ];
      ...
      if (
        !allSections.some((s) => s.id === 'references') &&
        caseStudy.references &&
        caseStudy.references.length > 0
      ) {
        allSections.push({
      ...
      allSections = allSections.filter((section) => {
        ...
        if (
          ...
          isMemoFicheSectionContentEmpty(section.data)
        )
          return false;
        return true;
      });
      ...
      const sortedSections =
        effectiveSectionOrder.length > 0
      ...
      const remainingSections = allSections.filter(
        (s) => !sortedSections.some((ss) => ss.id === s.id),
      );
      ...
      return finalSections.map((section, index) => ({
        ...
        icon: section.isMemoSection ? (
          ...
          <img
            src={getAbsoluteImageUrl(getIconUrl(section.id))}
            className="h-6 w-6 mr-3"
            alt={section.title}
          />
        ),
    ...
    const mainSections = [
      {
        ...
        icon: (
          <img
            src={getAbsoluteImageUrl(getIconUrl('patientSituation'))}
            className="h-6 w-6 mr-3"
            alt="Cas comptoir"
          />
        ),
        data:
          typeof caseStudy.patientSituation === 'string'
            ? caseStudy.patientSituation
            : caseStudy.patientSituation?.content,
        isAlert: false,
      },
      {
        id: 'keyQuestions',
        ...
        icon: (
          <img
            src={getAbsoluteImageUrl(getIconUrl('keyQuestions'))}
            className="h-6 w-6 mr-3"
            alt="Questions clés"
          />
        ),
      ...
      },
      {
        ...
        icon: (
          <img
            src={getAbsoluteImageUrl(getIconUrl('pathologyOverview'))}
            className="h-6 w-6 mr-3"
            alt="Aperçu pathologie"
          />
        ),
        data:
          typeof caseStudy.pathologyOverview === 'string'
            ? caseStudy.pathologyOverview
            : caseStudy.pathologyOverview?.content,
        isAlert: false,
      },
      {
        id: 'redFlags',
        ...
        icon: (
          <img
            src={getAbsoluteImageUrl(getIconUrl('redFlags'))}
            className="h-6 w-6 mr-3"
            alt="Signaux d'alerte"
          />
        ),
        data: caseStudy.redFlags,
        isAlert: true,
      },
      {
        id: 'mainTreatment',
        ...
        icon: (
          <img
            src={getAbsoluteImageUrl(getIconUrl('mainTreatment'))}
            className="h-6 w-6 mr-3"
            alt="Traitement principal"
          />
        ),
        data:
          caseStudy.mainTreatment && caseStudy.mainTreatment.length > 0
            ? caseStudy.mainTreatment
            : caseStudy.recommendations?.mainTreatment,
        isAlert: false,
      },
      {
        id: 'associatedProducts',
        ...
        icon: (
          <img
            src={getAbsoluteImageUrl(getIconUrl('associatedProducts'))}
            className="h-6 w-6 mr-3"
            alt="Produits associés"
          />
        ),
        data:
          caseStudy.associatedProducts &&
          caseStudy.associatedProducts.length > 0
            ? caseStudy.associatedProducts
            : caseStudy.recommendations?.associatedProducts,
        isAlert: false,
      },
      {
        id: 'lifestyleAdvice',
        title: caseStudy.lifestyleAdviceTitle || 'Hygiène de vie',
        icon: (
          <img
            src={getAbsoluteImageUrl(getIconUrl('lifestyleAdvice'))}
            className="h-6 w-6 mr-3"
            alt="Hygiène de vie"
          />
        ),
        data:
          caseStudy.lifestyleAdvice && caseStudy.lifestyleAdvice.length > 0
            ? caseStudy.lifestyleAdvice
            : caseStudy.recommendations?.lifestyleAdvice,
        isAlert: false,
      },
      {
        id: 'dietaryAdvice',
        ...
        icon: (
          <img
            src={getAbsoluteImageUrl(getIconUrl('dietaryAdvice'))}
            className="h-6 w-6 mr-3"
            alt="Conseils alimentaires"
          />
        ),
        data:
          caseStudy.dietaryAdvice && caseStudy.dietaryAdvice.length > 0
            ? caseStudy.dietaryAdvice
            : caseStudy.recommendations?.dietaryAdvice,
        isAlert: false,
      },
    ];
    ...
    const customSections = (caseStudy.customSections || []).map(
      (section, index) => ({
        id: section.id || `customSection-${index}`,
        title: section.title,
        icon: (
    ...
    const missingDefaultSectionIds = allAvailableSectionIds.filter(
      (id) => !effectiveSectionOrder.includes(id),
    );
    effectiveSectionOrder = [
      ...effectiveSectionOrder,
      ...missingDefaultSectionIds,
    ];
    ...
    const content = orderedSections.map((section) => {
      ...
      if (['mainTreatment', 'associatedProducts'].includes(section.id)) {
        ...
        renderedContent = renderContentWithKeywords(
          section.data,
          section.isAlert,
        );
      }
      ...
      icon: (
        <img
          src={getAbsoluteImageUrl(
            'https://pharmaconseilbmb.com/photos/site/icone/22.png',
          )}
          className="h-6 w-6 mr-3"
          alt="Références"
        />
  ...
  const menuItems: { id: TabName; label: string; icon: React.ReactNode }[] =
    useMemo(() => {
      const hasYoutubeLinks =
        caseStudy.youtubeLinks && caseStudy.youtubeLinks.length > 0;
      const hasFlashcards =
        caseStudy.flashcards && caseStudy.flashcards.length > 0;
      const hasQuiz = caseStudy.quiz && caseStudy.quiz.length > 0;
      const hasGlossary = caseStudy.glossary && caseStudy.glossary.length > 0;
      const hasQuizGemini =
        (caseStudy.type === 'savoir' || caseStudy.type === 'ordonnances') &&
        caseStudy.quizGeminiUrl;
      ...
      const isLeMedicamentManual =
        caseStudy.type === 'le-medicament' && !hasFlashcards && !hasQuiz;
      ...
      if (caseStudy.youtubeExplainerUrl) {
        items.push({
          ...
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('video-explainer'))}
              className="h-8 w-8"
              alt="Vidéo"
            />
          ),
      ...
      if (caseStudy.pdfSlideshowUrl) {
        items.push({
          ...
          icon: (
            <img
              src={getAbsoluteImageUrl(getIconUrl('diaporama'))}
              className="h-8 w-8"
              alt="Diaporama"
            />
          ),
      ...
      if (!isLeMedicamentManual) {
        if (hasMemoContent) {
          items.push({
            ...
            icon: (
              <img
                src={getAbsoluteImageUrl(getIconUrl('memo'))}
                className="h-8 w-8"
                alt="Mémo"
              />
            ),
        ...
        if (hasFlashcards) {
          items.push({
            id: 'flashcards' as TabName,
            label: 'Flashcards',
            icon: (
              <img
                src={getAbsoluteImageUrl(getIconUrl('flashcards'))}
                className="h-8 w-8"
                alt="Flashcards"
              />
            ),
        ...
        if (!isPreview && hasQuiz) {
          items.push({
            ...
            icon: (
              <img
                src={getAbsoluteImageUrl(getIconUrl('quiz'))}
                className="h-8 w-8"
                alt="Quiz"
              />
            ),
        ...
        if (!isPreview && caseStudy.kahootUrl) {
          items.push({
            id: 'kahoot' as TabName,
            label: 'Kahoot',
            icon: (
              <img
                src={getAbsoluteImageUrl(getIconUrl('kahoot'))}
                className="h-8 w-8"
                alt="Kahoot"
              />
            ),
        ...
        if (hasGlossary) {
          items.push({
            id: 'glossary' as TabName,
            label: 'Glossaire',
            icon: (
              <img
                src={getAbsoluteImageUrl(getIconUrl('glossary'))}
                className="h-8 w-8"
                alt="Glossaire"
              />
            ),
        ...
        if (hasYoutubeLinks) {
          // Note: this is for youtubeLinks, not youtubeExplainerUrl
          items.push({
            id: 'media' as TabName,
            label: 'Média',
            icon: (
              <img
                src={getAbsoluteImageUrl(getIconUrl('media'))}
                className="h-8 w-8"
                alt="Média"
              />
            ),
    ...
    switch (activeTab) {
      ...
      case 'flashcards':
        return (
          <FlashcardDeck
            ...
            memoFicheId={caseStudy._id as string}
          />
        );
      case 'glossary':
        return (
      ...
      case 'quiz':
        return (
          <div className="text-center bg-white p-8 rounded-lg shadow-md">
            ...
            <button
              onClick={onStartQuiz}
              className="inline-flex items-center bg-[#0B8278] text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-green-700"
            >
              <CheckCircleIcon className="h-6 w-6 mr-2" /> Démarrer le Quiz
            </button>
          </div>
        );
      case 'kahoot':
        return caseStudy.kahootUrl ? (
      ...
      case 'video-explainer':
        return caseStudy.youtubeExplainerUrl ? (
          ...
          <div className="text-center text-slate-500">
            Aucune vidéo explicative disponible.
          </div>
        );
      case 'diaporama':
        return caseStudy.pdfSlideshowUrl ? (
          ...
          <div className="text-center text-slate-500">
            Aucun diaporama PDF disponible.
          </div>
        );
      case 'artifact':
        return caseStudy.quizGeminiUrl ? (
          <div className="text-center bg-white p-8 rounded-lg shadow-md">
            <h3 className="text-2xl font-bold text-slate-800 mb-4">
              Testez vos connaissances avec le Quiz Gemini !
            </h3>
            <a
              href={caseStudy.quizGeminiUrl}
              target="_blank"
  ...
  return (
    <div className="animate-fade-in container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 rounded-lg overflow-hidden shadow-lg relative h-64 flex items-end p-8 text-white bg-slate-800">
          ...
          <div className="relative z-20">
            ...
            <div className="mt-2 text-sm font-medium opacity-90">
              ...
              <span className="mx-2">&bull;</span>
              <span>Statut: {caseStudy.status}</span>
            </div>
          </div>
        </div>
      ) : (
        ...
        <div className="lg:col-span-2">
          <div className="mb-6 border-b border-slate-200 flex space-x-1 sm:space-x-2 overflow-x-auto pb-px">
            {menuItems.map((item) => {
              ...
              const baseTabClass =
                'flex flex-col items-center px-3 py-3 text-sm sm:text-base font-medium rounded-t-md transition-all duration-300 border-b-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500';
              const activeTabClass =
                'bg-white text-teal-600 border-teal-600 shadow-sm';
              const inactiveTabClass =
                'border-transparent text-slate-500 hover:text-teal-500 hover:bg-slate-50';
              return (
                <button
                  key={item.id}
          ...
          <div className="mt-8 flex items-center justify-center space-x-4">
            {onBack && (
              <button
                onClick={onBack}
                className="px-6 py-3 text-base font-bold text-slate-700 bg-slate-200 rounded-lg hover:bg-slate-300"
              >
                Retour
              </button>
            )}
            {canEdit && onEdit && (
              <button
                onClick={onEdit}
                className="px-6 py-3 text-base font-bold text-white bg-teal-600 rounded-lg hover:bg-teal-700 flex items-center"
              >
            ...
            {canDelete && onDelete && (
              <button
                ...
                className="px-6 py-3 text-base font-bold text-white bg-red-600 rounded-lg hover:bg-red-700 flex items-center"
              >
            ...
            {canGenerateQRCode && (
              <button
                ...
                className="px-6 py-3 text-base font-bold text-white bg-slate-600 rounded-lg hover:bg-slate-700 flex items-center"
              >
          ...
          {caseStudy.theme === 'Dermatologie' && (
            <div className="mt-12 p-8 bg-pink-600 rounded-3xl text-white text-center shadow-xl animate-fade-in">
              ...
              <p className="mb-8 text-pink-100 max-w-md mx-auto">
                Mettez vos nouvelles connaissances en pratique avec une
                simulation finale pour valider votre progression.
              </p>
              <button
                onClick={() =>
                  navigate(`/apps/dermo/simulation/${caseStudy._id}`)
                }
              ...
              >
                <SimulationIcon className="h-6 w-6 mr-2" /> Lancer la Simulation
                Finale
              </button>
            </div>
          )}
          ...
          {showQRCode && canGenerateQRCode && (
            <div className="mt-6 flex flex-col items-center justify-center bg-white p-6 rounded-lg shadow-md animate-fade-in">
              ...
              <p className="mt-4 text-sm text-slate-600">
                Scannez ce code pour ouvrir cette page sur un autre appareil.
              </p>
            </div>
          )}
        </div>
        {(caseStudy.infographicImageUrl || premiumCampaign) && (
          <div className="lg:col-span-1 z-10 mt-8 lg:mt-20 space-y-8">
            {caseStudy.infographicImageUrl && (
              <div>
                <h3 className="text-lg font-bold text-slate-800 mb-4">
                  Infographie
                  ...
                  {caseStudy.infographicImageUrl.includes('canva.com') ? (
                    <div className="relative w-full h-full bg-slate-800 overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 z-10 bg-black/30 group-hover:bg-black/40 transition-colors">
                        <span className="text-white font-bold text-lg drop-shadow-md mb-2">
                          Infographie Synthétique
                        </span>
                        <span className="text-xs text-white/90 font-medium bg-black/50 px-3 py-1 rounded-full border border-white/20 backdrop-blur-sm">
            ...
            {premiumCampaign && (
              <PremiumSponsorBlock campaign={premiumCampaign} />
            )}
          </div>
        )}
      </div>
      ...
      {isInfographicModalOpen && (
        ...
        >
          <div
            className="w-full max-w-[95vw] h-[90vh] flex items-center justify-center relative"
...
const MemoFichePage = () => {
  ...
  const navigate = useNavigate();
  const { getCaseStudyById, startQuiz, editCaseStudy, deleteCaseStudy } =
    useData();
      ...
      try {
        ...
        if (user?.role === UserRole.PREPARATEUR) {
          ...
          if (groupRes.ok) {
            ...
            if (
              group.isPlanningEnabled &&
              group.planning &&
              group.planning.length > 0
            ) {
              const now = new Date();
              ...
              if (hasActivePlanning && !allowedIds.has(id)) {
                setErrorMessage(
                  "Votre pharmacien a activé un parcours guidé. Cette fiche n'est pas encore ouverte pour vous.",
                );
                setLoading(false);
                return;
              }
        ...
        if (err.message === 'Unauthorized') {
          ...
          setErrorMessage(
            'Une erreur est survenue lors du chargement de la mémofiche.',
          );
        }
      } finally {
        setLoading(false);
  ...
  if (!caseStudy) {
    // Fallback, should ideally be caught by errorMessage now
    return <Navigate to="/dashboard" replace />;
  }

  return (
    <DetailedMemoFicheView
      caseStudy={caseStudy}
