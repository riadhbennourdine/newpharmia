...
export const DetailedMemoFicheView: React.FC<DetailedMemoFicheViewProps> = ({
  ...
  const memoContent = useMemo(() => {
    ...
    if (caseStudy.memoSections && caseStudy.memoSections.length > 0) {
      allSections.push(
        ...caseStudy.memoSections.map((section, index) => ({
    ...
    if (caseStudy.type === 'maladie') {
      allSections.push(
        ...
        { id: 'pathologyOverview', title: caseStudy.pathologyOverviewTitle || 'Aperçu pathologie', data: caseStudy.pathologyOverview },
        { id: 'redFlags', title: caseStudy.redFlagsTitle || "Signaux d'alerte", data: caseStudy.redFlags, isAlert: true },
      );
      ...
      // ... (add other types as needed, mirroring the editor)
    }
    ...
    if (caseStudy.type !== 'dispositifs-medicaux' && caseStudy.type !== 'dermocosmetique' && caseStudy.type !== 'ordonnances' && caseStudy.type !== 'savoir' && caseStudy.type !== 'communication') {
        allSections.push(
            { id: 'mainTreatment', title: caseStudy.mainTreatmentTitle || 'Traitement Principal', data: caseStudy.mainTreatment },
            ...
            { id: 'keyPoints', title: caseStudy.keyPointsTitle || 'Points Clés & Références', data: caseStudy.keyPoints }
        );
    }
    ...
    // References, always last unless ordered otherwise
    ...
    const unOrderedSections = allSections.filter(section => !orderedSections.some(os => os.id === section.id));

    const finalSections = [...orderedSections, ...unOrderedSections];
