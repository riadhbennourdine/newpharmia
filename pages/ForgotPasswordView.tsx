    ...
    try {
      ...
      if (response.ok) {
        if (data.migrationRequired) {
          ...
          setMessage(
            data.message ||
              'Si votre compte est trouvé, des instructions ont été envoyées.',
          );
        }
      } else {
        setError(
    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-50 font-sans">
      <div className="w-full max-w-md p-8 space-y-8 bg-white rounded-xl shadow-lg">
        <div className="text-center">
          <h2 className="mt-2 text-xl text-slate-600">Mot de passe oublié ?</h2>
          <p className="text-sm text-slate-500 mt-2">
            Entrez votre adresse e-mail ou votre pseudo pour recevoir un lien de
            réinitialisation.
          </p>
        </div>
