...
const DermoSimulationPage: React.FC = () => {
            ...
            getCaseStudyById(id).then(data => {
                ...
                if (data) {
                    const initialGreeting = Array.isArray(data.patientSituation?.content)
                        ? data.patientSituation.content.find((c: any) => c.type === 'text')?.value || "Bonjour, je viens vous voir car j'ai quelque chose sur la peau..."
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            setMessages([...newMessages, { role: 'model', text: "Le patient semble confus... (Erreur service)" }]);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    if (isFicheLoading) return <div className="flex justify-center items-center h-screen"><Spinner className="h-12 w-12 text-pink-600" /></div>;
    if (!fiche) return <div className="flex justify-center items-center h-screen">Fiche introuvable.</div>;
        ...
        <div className="flex flex-col h-[calc(100vh-64px)] bg-slate-100 font-sans">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between shadow-sm z-10">
                <div className="flex items-center">
                    <button onClick={() => navigate('/apps/dermo')} className="mr-4 text-slate-400 hover:text-pink-600 transition-colors">
                        <ArrowLeftIcon className="h-6 w-6" />
                    </button>
                    <div>
                        <h2 className="text-lg font-bold text-slate-800 leading-tight">Cas : {fiche.title}</h2>
                        <p className="text-xs text-slate-500 uppercase font-bold tracking-widest">Simulation Interactive</p>
                    </div>
                </div>
                {!isEvaluationMode && (
                    <button 
            ...
            <div className="flex flex-grow overflow-hidden">
                ...
                <div className="hidden md:flex flex-col w-1/3 border-r border-slate-200 bg-white p-6 overflow-y-auto">
                    <h3 className="text-xs font-black text-slate-400 uppercase mb-4 tracking-widest">Observation Clinique</h3>
                    <div className="rounded-2xl overflow-hidden shadow-lg border border-slate-100 bg-slate-50 relative group">
                        {lesionImage ? (
                            <img src={getAbsoluteImageUrl(lesionImage)} alt="Lésion cutanée" className="w-full h-auto object-cover transition-transform duration-500 group-hover:scale-105" />
                        ) : (
                            <div className="p-16 text-center text-slate-300 flex flex-col items-center">
                                <span className="text-6xl mb-4">🧴</span>
                                <p className="text-sm font-medium italic">Image non disponible pour ce cas.</p>
                            </div>
                        )}
                    </div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {[
                                    ...
                                    <div key={item.l} className="flex items-center space-x-2">
                                        <span className="w-6 h-6 flex items-center justify-center bg-white rounded-full text-[10px] font-black text-pink-600 shadow-sm">{item.l}</span>
                                        <span className="text-[11px] font-bold text-pink-700">{item.t}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <p className="text-[11px] text-slate-400 leading-relaxed italic text-center px-4">
                            Questionnez le patient pour identifier les lésions élémentaires et exclure les drapeaux rouges.
                        </p>
                    </div>
                </div>
                            ...
                            <div className="max-w-2xl mx-auto">
                                ...
                                <div className="space-y-6">
                                    <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100 relative">
                                        <span className="absolute -top-3 left-8 bg-teal-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">Feedback du Coach</span>
                                        <p className="text-slate-700 leading-relaxed text-lg italic">"{evaluation?.feedback}"</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                                        ...
                                        <button 
                                            onClick={() => { setMessages([{role:'model', text: messages[0].text}]); setIsEvaluationMode(false); setEvaluation(null); }}
                        ...
                        <>
                            ...
                            <div className="p-4 bg-white border-t border-slate-200 shadow-[0_-4px_20px_-2px_rgba(0,0,0,0.05)]">
                                <div className="max-w-4xl mx-auto flex items-end space-x-3">
                                    ...
                                    <button
                                        ...
                                        className={`p-3.5 rounded-xl transition-all ${
                                            isLoading || !input.trim() ? 'bg-slate-200 text-slate-400' : 'bg-pink-600 text-white shadow-lg hover:bg-pink-700 active:scale-95'
                                        }`}
                                    >
                                ...
                                <div className="text-center mt-2">
                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Objectif : Appliquer le P.H.A.R.M.A</p>
                                </div>
                            </div>
                        </>
                    )}
