    ...
    const fetchDermoFiches = async () => {
      ...
      try {
        ...
        const response = await fetch(
          '/api/memofiches?theme=Dermatologie&limit=100',
        );
        if (!response.ok) throw new Error('Fetch failed');
        const data = await response.json();
        console.log(
          `[DermoGuide] Fetched ${data.data?.length} fiches with theme Dermatologie`,
        );
        setFiches(data.data || []);
      } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center">
            <button
              ...
              className="mr-4 text-gray-500 hover:text-pink-600 transition-colors"
            >
            ...
            <h1 className="text-xl font-bold text-gray-900 font-poppins flex items-center">
              ...
              <span className="ml-2 text-sm font-normal text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full hidden sm:inline-block">
                Approche PHARMA
              </span>
            </h1>
          </div>
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-10 text-center">
          <h2 className="text-3xl font-extrabold text-slate-900 font-poppins">
            Catalogue DermoGuide
          </h2>
          <p className="mt-3 text-lg text-slate-600 max-w-3xl mx-auto">
            Priorité à la simulation : Entraînez-vous d'abord, accédez à la
            fiche ensuite pour progresser.
          </p>
        </div>
        ...
        {isLoading ? (
                ...
                <section key={group.id} className="animate-fade-in">
                  <div className="flex items-center mb-8">
                    ...
                    <span className="ml-4 px-2.5 py-0.5 rounded-full text-xs font-bold bg-pink-100 text-pink-700 uppercase">
                      {groupFiches.length}{' '}
                      {groupFiches.length > 1 ? 'Fiches' : 'Fiche'}
                    </span>
                  </div>
          ...
          <div className="text-center py-20 bg-white rounded-2xl border-2 border-dashed border-slate-200">
            ...
            <h3 className="text-xl font-bold text-slate-800">
              Aucune fiche DermoGuide disponible
            </h3>
            <p className="text-slate-500 mt-2">
              Utilisez le Générateur DermoGuide pour créer vos premières fiches.
            </p>
            <button
...
const DermoFicheCard: React.FC<{ fiche: CaseStudy; user: any }> = ({
  fiche,
  user,
}) => {
  const navigate = useNavigate();
  ...
  const sims =
    user?.simulationHistory?.filter((s: any) => s.ficheId === fiche._id) || [];
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2">
          ...
          {postReadSim && (
            <div className="bg-teal-500/90 backdrop-blur px-2 py-1 rounded-lg shadow-md border border-teal-400 flex flex-col items-center">
              <span className="text-[9px] font-black text-white uppercase leading-none mb-1">
                Final
              </span>
        ...
        {preReadSim && postReadSim && (
          <div className="mb-4 flex items-center gap-2 p-2 bg-teal-50 rounded-lg border border-teal-100">
            ...
            <span className="text-xs font-bold text-teal-800">
              Progression :{' '}
              {postReadSim.score - preReadSim.score > 0 ? '+' : ''}
              {postReadSim.score - preReadSim.score}%
            </span>
          </div>
        )}
