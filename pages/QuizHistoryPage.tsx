...
const QuizHistoryPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchQuizHistory = async () => {
            ...
            try {
                ...
                const historyResponse = await fetch(`/api/users/${userId}/quiz-history`);
                ...
                if (rawQuizHistory && rawQuizHistory.length > 0) {
                    ...
                    const quizzesWithTitles = await Promise.all(rawQuizHistory.map(async (entry: QuizEntry) => {
                        try {
                            const memoficheResponse = await fetch(`/api/memofiches/${entry.quizId}`);
                            const memoficheData = await memoficheResponse.json();
                            return { ...entry, quizTitle: memoficheData.title };
                        } catch (memoficheError) {
                            console.error(`Error fetching memofiche for quizId ${entry.quizId}:`, memoficheError);
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <h1 className="text-3xl font-bold text-slate-800 mb-6">Historique des quiz de {userName}</h1>
                    ...
                    {quizHistory.map((entry, index) => (
                        <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                            <Link to={`/memofiche/${entry.quizId}`}>
                                <div className="p-4">
                                    <h3 className="font-bold text-lg text-slate-800 mb-2">{entry.quizTitle || 'Quiz'}</h3>
                                    <p className="text-sm text-slate-600">Score: {entry.score}%</p>
                                    <p className="text-sm text-slate-600">Date: {new Date(entry.completedAt).toLocaleDateString('fr-FR')}</p>
                                </div>
                            </Link>
                        </div>
                    ))}
