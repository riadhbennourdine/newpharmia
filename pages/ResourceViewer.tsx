...
const ResourceViewer: React.FC = () => {
  ...
  const getIconForResourceType = (type: string) => {
    switch (type) {
      case 'diaporama':
        return <ArrowDownOnSquareIcon className="h-8 w-8 text-teal-600" />;
      case 'infographie':
  ...
  if (error)
    return (
      <div className="text-center p-10 bg-red-50 text-red-700">{error}</div>
    );
  if (!resourcePage)
    return (
      <div className="text-center p-10">Page de ressource non trouvée.</div>
    );
                  ...
                  {resourcePage.resources.map((resource, index) => (
                    ...
                    >
                      ...
                      <div className="flex-grow">
                        <p className="font-semibold text-lg text-gray-900 group-hover:text-teal-700 transition-colors">
                          {resource.title}
                        </p>
