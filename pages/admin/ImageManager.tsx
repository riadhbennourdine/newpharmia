...
const ImageManager: React.FC = () => {
  ...
  const handleDelete = async (
    ...
    if (
      !window.confirm(
        `Êtes-vous sûr de vouloir supprimer "${itemName}" (${itemType}) ?`,
      )
    )
      return;
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || `Failed to delete ${itemType} from FTP.`,
        );
      }

      await fetchFtpItems(); // Refresh the list
      alert(
        `${itemType === 'file' ? 'Fichier' : 'Dossier'} supprimé avec succès !`,
      );
    } catch (err: any) {
      setError(err.message);
    } finally {
  ...
  const handleMove = async (itemName: string) => {
    const destination = window.prompt(
      'Entrez le chemin du dossier de destination (ex: /images/promo):',
      currentPath,
    );
    ...
    try {
      const response = await fetch('/api/ftp/rename', {
        // Use rename endpoint for moving
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {ftpItems.map((item) => (
              <div
                key={item.name}
                className="group bg-slate-50 rounded-lg shadow-sm overflow-hidden flex flex-col"
              >
                {item.type === 'directory' ? (
                  <button
                    onClick={() => handleNavigate(item.name, item.type)}
                    className="w-full h-32 flex items-center justify-center bg-blue-100 text-blue-600 text-5xl hover:bg-blue-200"
                  >
                    📁
                  </button>
                ) : (
