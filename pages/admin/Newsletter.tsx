              ...
              >
                <tbody>
                  ...
                  <tr>
                    ...
                    >
                      <img
                        src="https://pharmaconseilbmb.com/photos/site/23.png"
                        alt="Favicon"
                        width={30}
                        style={{ marginRight: '10px', verticalAlign: 'middle' }}
                      />
                      <img
                        src="https://pharmaconseilbmb.com/photos/site/logo-pharmia.png"
                        alt="PharmIA Logo"
                        width={150}
                  ...
                  <tr>
                    ...
                    >
                      <p style={{ marginTop: 0, marginBottom: 8 }}>
                        <a
                          href="https://newpharmia-production.up.railway.app/"
                          style={{
                            color: '#0d9488',
                            textDecoration: 'none',
                            fontWeight: 'bold',
                        ...
                        | By PharmaConseil BMB - Formation continue à
                        l\'officine
                      </p>
                      {/* FIX: Changed margin from a string to a number to resolve TypeScript error. */}
                      <p style={{ margin: 0 }}>
                        <a
                          href={`/#/unsubscribe?email={{EMAIL_DESTINATAIRE}}`}
...
const ExpiredTrialTemplate: React.FC<TemplateProps> = ({ recipientName }) => {
  const surveyLink = (score: number) =>
    `https://newpharmia-production.up.railway.app/api/survey/rating?score=${score}&userId={{USER_ID}}`;
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    >
                      ...
                      >
                        Votre expert du conseil officinal.
                      </p>
                    </td>
                  </tr>
                  {/* Main Content */}
                  <tr>
                    <td style={{ padding: '32px', color: '#111827' }}>
                      ...
                      >
                        ...
                        <br />
                        Votre période d'essai de 7 jours sur PharmIA est arrivée
                        à son terme. Nous espérons que vous avez pu découvrir la
                        richesse de nos contenus et l'efficacité de notre
                        plateforme pour renforcer votre expertise au comptoir.
                        <br />
                        <br />
                        Pour continuer à bénéficier d'un accès illimité à toutes
                        nos mémofiches, quiz interactifs et parcours
                        d'apprentissage, nous vous invitons à choisir la formule
                        d'abonnement qui vous convient.
                      </p>
                        ...
                        <tbody>
                          <tr>
                            ...
                            >
                              ...
                              >
                                Engagez-vous pour un an et bénéficiez de{' '}
                                <strong>3 mois gratuits !</strong> C'est
                                l'occasion idéale pour maîtriser le conseil à
                                l'officine tout au long de l'année.
                              </p>
                            </td>
                          </tr>
                        </tbody>
                      ...
                      >
                        <tbody>
                          <tr>
                            <td align="center">
                              <a
                                href="https://newpharmia-production.up.railway.app/#/tarifs"
                        ...
                        <tbody>
                          <tr>
                            <td style={{ padding: '20px' }}>
                              ...
                              >
                                Quelle est la probabilité que vous recommandiez
                                PharmIA à un confrère ?
                              </p>
                              <table
                                cellPadding="0"
                                cellSpacing="0"
...
const Newsletter: React.FC = () => {
  ...
  const [selectedFormalGroupIds, setSelectedFormalGroupIds] = useState<
    string[]
  >([]);
  ...
  >([]);
  const [selectedTestEmails, setSelectedTestEmails] = useState<
    { value: string; label: string }[]
  >([]);
  ...
  const templates: Template[] = [
    ...
    {
      id: 'expired-trial',
      name: 'Relance Essai Expiré',
      component: ExpiredTrialTemplate,
    },
  ];
    ...
    const fetchInitialData = async () => {
      try {
        ...
        const subscriberResponse = await fetch(
          '/api/newsletter/subscriber-groups',
        );
        if (!subscriberResponse.ok) {
          throw new Error('Failed to fetch subscriber groups');
        }
        const { roles, cities, staff, statuses } =
  ...
  const handleSendTest = async () => {
    if (!previewRef.current) {
      setSendStatus(
        'Erreur: Impossible de récupérer le contenu de la prévisualisation.',
      );
      return;
    }
    if (selectedTestEmails.length === 0) {
      setSendStatus(
        'Veuillez sélectionner au moins une adresse e-mail de test.',
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.message || "Échec de l'envoi de l'e-mail de test.",
        );
      }
      ...
      setSendStatus(
        error.message || "Erreur lors de l'envoi de l'e-mail de test.",
      );
    } finally {
    ...
    try {
      const response = await fetch('/api/newsletter/send', {
        ...
        body: JSON.stringify({
          ...
          webinarId: selectedWebinar ? selectedWebinar.value : null,
          googleMeetLink: selectedWebinar
            ? selectedWebinar.googleMeetLink
            : null,
          sendToExpired,
        }),
      });
      ...
      setSendStatus(
        error.message || "Erreur lors de l'envoi de la newsletter.",
      );
    } finally {
      setTimeout(() => setSendStatus(''), 5000);
    }
        ...
        <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {staff && (
                <div key={staff.name} className="flex items-center">
                  <input
                    type="checkbox"
                    id={`role-select-${staff.name}`}
                    checked={selectedRoles.includes(staff.name)}
                    onChange={() => handleRoleToggle(staff.name)}
                    className="h-4 w-4 text-teal-600 border-gray-300 rounded focus:ring-teal-500"
                    disabled={!!selectedWebinar || sendToExpired}
                  />
                  <label
                    htmlFor={`role-select-${staff.name}`}
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {statuses.map((status) => (
                <div key={status.name} className="flex items-center">
                  ...
                  />
                  <label
                    htmlFor={`status-select-${status.name}`}
                    className="ml-2 text-sm text-gray-700"
                  >
          ...
          <div className="mb-4">
            ...
            <Select
              ...
              isDisabled={!!selectedWebinar || sendToExpired}
            />
            <p className="text-xs text-gray-500 mt-1">
              Si aucun groupe n\'est sélectionné, la newsletter sera envoyée à
              tous les abonnés.
            </p>
          </div>
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700">
              Envoyer aux groupes formels
            </label>
            <Select
              isMulti
              options={formalGroups.map((g) => ({
                value: g._id,
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700">
              Envoyer aux apprenants à période expirée
            </label>
            <div className="mt-2 flex items-center">
              <input
                type="checkbox"
                id="expired-trial-select"
                checked={sendToExpired}
                onChange={(e) => handleExpiredTrialToggle(e.target.checked)}
                className="h-4 w-4 text-teal-600 border-gray-300 rounded focus:ring-teal-500"
                disabled={!!selectedWebinar}
              />
              <label
                htmlFor="expired-trial-select"
                className="ml-2 text-sm text-gray-700"
              >
                Apprenants à période expirée ({expiredTrialUsers.length})
              </label>
            </div>
          </div>
          ...
          <div className="mb-4 p-4 border-t border-b border-gray-200">
            <label className="block text-sm font-medium text-gray-700">
              Ou envoyer aux participants d\'un webinaire
            </label>
            <Select
              options={webinars}
              isClearable
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Ceci désactivera les autres options de ciblage.
            </p>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="mt-1 mb-2 flex flex-wrap gap-2">
              <button
                onClick={() => insertTag('NOM_DESTINATAIRE')}
                className="px-2 py-1 text-xs bg-gray-200 rounded hover:bg-gray-300"
              >
                Nom du destinataire
              </button>
              <button
                onClick={() => insertTag('YOUTUBE_VIDEO')}
                className="px-2 py-1 text-xs bg-gray-200 rounded hover:bg-gray-300"
              >
                Vidéo YouTube
              </button>
              <button
                onClick={() => insertTag('IMAGE_URL')}
                className="px-2 py-1 text-xs bg-gray-200 rounded hover:bg-gray-300"
              >
                Image URL
              </button>
              {selectedWebinar && (
                <button
                  onClick={() => insertTag('WEBINAR_DESCRIPTION')}
                  className="px-2 py-1 text-xs bg-gray-200 rounded hover:bg-gray-300"
                >
          ...
          <button
            onClick={handleSend}
            className="bg-teal-600 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded-lg transition-colors w-full"
          >
          ...
          <div className="border rounded-lg overflow-hidden">
            <div className="p-4 bg-gray-100">
              <p className="text-sm text-gray-600">
                De: Votre Nom &lt;newsletter@pharmia.fr&gt;
              </p>
