  ...
  const handleFileUpload = async (userId: string, file: File) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/subscriptions/${userId}/upload-proof`,
        {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold text-slate-800 mb-6">
        Gestion des Abonnements & Justificatifs
      </h1>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {sub.latestOrder ? (
                    <div>
                      ...
                      <div
                        className={`font-bold ${sub.latestOrder.status === 'CONFIRMED' ? 'text-green-600' : 'text-orange-600'}`}
                      >
