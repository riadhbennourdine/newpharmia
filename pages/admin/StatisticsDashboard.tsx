...
const StatisticsDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAllStats = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            'Impossible de charger les statistiques rétrospectives.',
          );
        }
  ...
  if (loading)
    return (
      <div className="text-center p-8">Chargement des statistiques...</div>
    );
  if (error)
    return <div className="text-center p-8 text-red-500">Erreur: {error}</div>;
  if (!stats)
    return (
      <div className="text-center p-8">Aucune statistique à afficher.</div>
    );
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-white p-6 rounded-xl shadow mb-8">
        ...
        <div className="flex justify-center gap-2 mb-4">
          {(['daily', 'weekly', 'monthly'] as Period[]).map((p) => (
            <button
              key={p}
              onClick={() => setEventPeriod(p)}
              className={`px-4 py-2 text-sm font-medium rounded-md ${eventPeriod === p ? 'bg-teal-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
            >
              {p === 'daily'
                ? 'Journalier (30j)'
                : p === 'weekly'
        ...
        {loadingEvents ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-6">
            <div>
              ...
              <Line
                data={getChartData(loginData, 'Connexions', '52, 211, 153')}
              />
            </div>
            <div>
              <h3 className="text-lg font-semibold text-slate-600 mb-2">
      ...
      <h2 className="text-2xl font-semibold text-slate-700 mb-4 mt-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow col-span-1 md:col-span-2 lg:col-span-2">
          <h3 className="text-lg font-semibold text-slate-600 mb-2">
            Total de lectures de mémofiches
          </h3>
          <p className="text-4xl font-bold text-teal-600">
            {stats.totalFicheReads}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow mb-8">
        ...
        <Bar
          data={{
            labels: stats.registrationsByMonth.map(
              (item) =>
      ...
      <div className="bg-white p-6 rounded-xl shadow">
        <h3 className="text-lg font-semibold text-slate-600 mb-4">
          Top 10 des mémofiches les plus lues
        </h3>
        <Bar
          data={{
