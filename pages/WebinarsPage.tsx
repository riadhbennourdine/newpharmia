...
const WebinarsPage: React.FC = () => {
  ...
  const [masterClassDescription, setMasterClassDescription] =
    useState<string>('');
  ...
  const [activePharmiaPricingTab, setActivePharmiaPricingTab] =
    useState<string>('PIA_PACK_4'); // Default to "Pack 4 Crédits"
  ...
  const [futureMonthsWebinars, setFutureMonthsWebinars] = useState<
    Record<string, Webinar[]>
  >({});
  ...
  const [selectedWebinarForResources, setSelectedWebinarForResources] =
  ...
  const handleUseCredit = async (
    webinarId: string,
    webinarGroup: WebinarGroup,
  ) => {
    if (!token) return;

    const creditType =
      webinarGroup === WebinarGroup.MASTER_CLASS ? 'Master Class' : 'PharmIA';
    if (
      !window.confirm(
        `Voulez-vous utiliser 1 crédit ${creditType} pour vous inscrire ?`,
      )
    )
      return;
    ...
    try {
      ...
      const allPharmIAWebinars = await fetchWebinars(
        token,
        WebinarGroup.PHARMIA,
      );
      const allMasterClassWebinars = await fetchWebinars(
        token,
  ...
  const calculateStatus = useCallback(
    (date: string, duration: number = 90, group?: string): WebinarStatus => {
      const now = new Date();
  ...
  const processWebinars = useCallback(
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setError(
          `Impossible de charger les webinaires. Raison: ${errorMessage}`,
        );
        console.error(err);
      } finally {
        setIsLoading(false);
  ...
  const handleSaveResources = async (
    ...
    if (
      !user ||
      ![UserRole.ADMIN, UserRole.ADMIN_WEBINAR].includes(user.role)
    ) {
    ...
    try {
      ...
      if (user && token) {
        const myW = await fetchMyWebinars(token);
        const filteredMyW = myW.filter(
          (w) => w.group === WebinarGroup.CROP_TUNIS,
        );
        setMyRegisteredWebinars(processWebinars(filteredMyW));
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    if (view === 'MY_WEBINARS_CROP') {
      tabSpecificWebinars = myRegisteredWebinars.filter(
        (w) => w.group === WebinarGroup.CROP_TUNIS,
      );
    } else if (view === 'MY_WEBINARS_PHARMIA') {
      tabSpecificWebinars = myRegisteredWebinars.filter(
        (w) => w.group === WebinarGroup.PHARMIA,
      );
    } else if (
      view === 'PHARMIA_PHARMACIEN' ||
      view === 'PHARMIA_PREPARATEUR'
    ) {
      tabSpecificWebinars = allWebinars.filter(
        (w) => w.group === WebinarGroup.PHARMIA,
      );
    } else {
    ...
    const live = processedTabWebinars.filter(
      (w) => w.calculatedStatus === WebinarStatus.LIVE,
    );
    const upcoming = processedTabWebinars
      .filter(
        (w) =>
          w.calculatedStatus === WebinarStatus.UPCOMING ||
          w.calculatedStatus === WebinarStatus.REGISTRATION_CLOSED,
      )
    ...
    const groupedFutureWebinars = futureMonthsUpcoming.reduce(
      (acc, webinar) => {
    ...
    <div className="space-y-12">
      <div className="text-center">
        ...
        <p className="text-xl text-slate-600 max-w-3xl mx-auto">
          ...
          conçus pour les pharmaciens et préparateurs en pharmacie.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-center">
        ...
        <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden flex flex-col p-8 border border-slate-200">
          <div className="flex-grow">
            <img
              src="/api/ftp/view?filePath=%2Fpharmia%2Fcropt%2Fcrop-tunis.jpg"
              alt="CROP Tunis Logo"
              className="h-24 w-auto mx-auto mb-4 object-contain"
            />
            <h3 className="text-2xl font-bold text-slate-800 mb-2">
              Wébinaires CROP Tunis
            </h3>
            <p className="text-sm font-semibold text-teal-600 mb-4">
              Pour Préparateurs
            </p>
            <p className="text-slate-600 mb-6 flex-grow">
              Le programme de formation de référence pour les préparateurs en
              pharmacie d'officine.
            </p>
          </div>
          <div className="mt-auto">
            <button
              onClick={() => setView(WebinarGroup.CROP_TUNIS)}
              className="w-full text-white font-bold py-3 px-4 rounded-lg transition-colors shadow-md hover:shadow-lg"
              style={{
                backgroundColor: '#197738',
                '&:hover': { backgroundColor: '#15652D' },
              }}
            >
        ...
        <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden flex flex-col p-8 border border-slate-200">
          <div className="flex-grow">
            ...
            <p className="text-sm font-semibold text-teal-600 mb-4">
              Pour Pharmaciens & Préparateurs
            </p>
            <p className="text-slate-600 mb-6 flex-grow">
              Des sessions interactives pour maîtriser les outils et la
              méthodologie PharmIA au comptoir.
            </p>
          </div>
          <div className="mt-auto">
            <button
              onClick={() => setView(WebinarGroup.PHARMIA)}
              className="w-full bg-teal-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-teal-700 transition-colors shadow-md hover:shadow-lg"
            >
              Découvrir les sessions
            </button>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden flex flex-col p-8 border border-slate-200">
          <div className="flex-grow">
            ...
            <p className="text-slate-600 mb-6 flex-grow">
              Un programme d'excellence pour approfondir vos connaissances sur
              des thèmes majeurs de l'officine.
            </p>
          </div>
          <div className="mt-auto">
            <button
              onClick={() => setView(WebinarGroup.MASTER_CLASS)}
              className="w-full bg-teal-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-teal-700 transition-colors shadow-md hover:shadow-lg"
            >
      ...
      return (
        <div className="text-center py-12 bg-white rounded-lg shadow-sm">
          <h3 className="text-xl font-semibold text-slate-700">
            Aucune Master Class disponible
          </h3>
    ...
    return (
      <div className="space-y-16 max-w-7xl mx-auto">
        {sortedGroupKeys.map((groupTitle) => {
          const sessions = grouped[groupTitle].sort(
            (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime(),
          );
          const firstSession = sessions[0];
          const isPast = sessions.every(
            (s) => s.calculatedStatus === WebinarStatus.PAST,
          );

          return (
            <div
              key={groupTitle}
              className={`rounded-2xl border transition-all duration-300 ${isPast ? 'border-slate-200 bg-slate-50/50 opacity-80' : 'border-teal-100 bg-white shadow-lg shadow-teal-900/5'}`}
            >
              {/* Header of the MC Group */}
              <div
                className={`px-8 py-6 border-b rounded-t-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${isPast ? 'border-slate-200 bg-slate-100/50' : 'border-teal-100 bg-gradient-to-r from-teal-50/50 to-white'}`}
              >
                <div>
                  <h3
                    className={`text-2xl font-extrabold tracking-tight ${isPast ? 'text-slate-600' : 'text-slate-800'}`}
                  >
                    {groupTitle}
                  </h3>
                  <p className="text-slate-500 text-sm mt-1 font-medium flex items-center gap-2">
                    Cycle de 3 sessions • Début le{' '}
                    {new Date(firstSession.date).toLocaleDateString('fr-FR', {
                      day: 'numeric',
                      month: 'long',
                      year: 'numeric',
                    })}
                ...
                {isPast && (
                  <span className="px-4 py-1.5 bg-slate-200 text-slate-600 rounded-full text-xs font-bold uppercase tracking-wide">
                    Terminé
                  </span>
                )}
              </div>
                  ...
                  {sessions.map((webinar, idx) => (
                    <div key={webinar._id.toString()} className="relative">
                      <WebinarCard
                        ...
                        onUseCredit={(webinarId) =>
                          handleUseCredit(webinarId, webinar.group)
                        }
                      />
                    </div>
                  ))}
  ...
  const renderWebinarList = (
    ...
    if (webinarsToRender.length === 0) {
      return (
        <div className="text-center py-12 bg-white rounded-lg shadow-sm">
          <h3 className="text-xl font-semibold text-slate-700">
            Aucun wébinaire pour le moment
          </h3>
          <p className="text-slate-500 mt-2">
      ...
      <div className="space-y-12">
        {liveWebinars.length > 0 && !isMyList && (
          <div className="mb-12">
            ...
            <div
              className="p-6 text-slate-800 rounded-lg shadow-xl"
              style={{ backgroundColor: '#CBDFDE' }}
            >
              <div className="grid grid-cols-1 gap-6">
                {liveWebinars.map((webinar) => (
                  <WebinarCard
                    ...
                    onResourceClick={handleResourceClick}
                    onManageResources={
                      canManageResources ? handleOpenResourcesModal : undefined
                    }
                    userCredits={user?.masterClassCredits}
                    pharmiaCredits={user?.pharmiaCredits}
                    onUseCredit={(webinarId) =>
                      handleUseCredit(webinarId, webinar.group)
                    }
                  />
        ...
        {nearestWebinar && !isMyList && (
          <div
            className="mb-12 p-6 rounded-lg shadow-xl border-2 border-teal-500 max-w-4xl"
            style={{ backgroundColor: '#CBDFDE' }}
          >
            ...
            <WebinarCard
              ...
              pharmiaCredits={user?.pharmiaCredits}
              onUseCredit={(webinarId) =>
                handleUseCredit(webinarId, nearestWebinar.group)
              }
            />
          </div>
        )}
        ...
        {Object.entries(futureMonthsWebinars).length > 0 && !isMyList && (
          <div>
            ...
            {Object.entries(futureMonthsWebinars).map(
              ([monthYear, monthWebinars]) => (
    ...
    <div className="bg-slate-50 min-h-screen">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {(view === WebinarGroup.CROP_TUNIS ||
              ...
              <div className="mt-6 p-4 border rounded-lg bg-blue-50 border-blue-200">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  {user && (
                    <button
                      onClick={() => setView('MY_WEBINARS_CROP')}
                      className="w-full sm:w-auto text-blue-600 hover:text-blue-800 font-medium py-2 px-4 rounded-lg hover:bg-blue-100 transition-colors border border-blue-400"
                    >
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="w-full sm:w-auto text-red-600 hover:text-red-800 font-medium py-2 px-4 rounded-lg hover:bg-red-100 transition-colors border border-red-400"
                    >
            ...
            {(view === WebinarGroup.PHARMIA ||
              ...
              <div className="mt-6 p-4 border rounded-lg bg-teal-50 border-teal-200">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  {user && !isWebinarAdmin && (
                    <button
                      onClick={() => setView('MY_WEBINARS_PHARMIA')}
                      className="w-full sm:w-auto text-teal-600 hover:text-teal-800 font-medium py-2 px-4 rounded-lg hover:bg-teal-100 transition-colors border border-teal-400"
                    >
                  ...
                  <button
                    onClick={() => {
                      console.log(
                        'Toggling isPharmiaPricingOpen:',
                        !isPharmiaPricingOpen,
                      );
                      setIsPharmiaPricingOpen(!isPharmiaPricingOpen);
                    }}
                    className="w-full sm:w-auto flex items-center justify-center gap-2 text-slate-600 font-bold py-2 px-4 rounded-lg hover:bg-slate-200 transition-colors border border-slate-400"
                  >
        ...
        {isLoading ? (
          ...
          myRegisteredWebinars.filter(
            ...
            <div className="text-center py-12 bg-white rounded-lg shadow-sm">
              <h3 className="text-xl font-semibold text-slate-700">
                Vous n'êtes inscrit à aucun wébinaire CROP Tunis pour le moment
              </h3>
              <p className="text-slate-500 mt-2">
                Découvrez nos prochaines sessions CROP Tunis et inscrivez-vous !
              </p>
            </div>
          )
        ) : view === 'MY_WEBINARS_PHARMIA' ? (
          myRegisteredWebinars.filter((w) => w.group === WebinarGroup.PHARMIA)
            .length > 0 ? (
            <div className="space-y-12">
              <h2 className="text-3xl font-bold text-slate-800 mb-4">
                Mes wébinaires PharmIA
            ...
            <div className="text-center py-12 bg-white rounded-lg shadow-sm">
              <h3 className="text-xl font-semibold text-slate-700">
                Vous n'êtes inscrit à aucun wébinaire PharmIA pour le moment
              </h3>
              <p className="text-slate-500 mt-2">
                Découvrez nos prochaines sessions PharmIA et inscrivez-vous !
              </p>
            </div>
          )
        ) : view === 'PHARMIA_PHARMACIEN' ? (
          renderWebinarList(
            allWebinars.filter(
              (w) =>
                w.group === WebinarGroup.PHARMIA &&
                (w.targetAudience === 'Pharmacien' ||
                  w.targetAudience === 'Tous'),
            ),
          )
        ) : view === 'PHARMIA_PREPARATEUR' ? (
          renderWebinarList(
      ...
      {isPharmiaPricingOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100 relative">
            {' '}
            ...
            <div className="flex flex-wrap border-b border-slate-200 bg-slate-50">
              {PHARMIA_CREDIT_PACKS.map((pack) => (
                ...
                >
                  ...
                  {pack.badge && (
                    <span
                      className={`block mt-1 text-[10px] uppercase tracking-wide ${activePharmiaPricingTab === pack.id ? 'text-teal-500' : 'text-slate-400'}`}
                    >
                      {pack.badge}
                    </span>
                  )}
            ...
            <div className="p-8 md:p-12">
              {PHARMIA_CREDIT_PACKS.map((pack) => {
                ...
                return (
                  <div
                    key={pack.id}
                    className="flex flex-col md:flex-row gap-8 items-center md:items-start animate-fadeIn"
                  >
                    <div className="flex-1 text-center md:text-left">
                      ...
                      <div className="mb-6 inline-block bg-teal-50 rounded-xl p-6 border border-teal-100">
                        ...
                        <div className="flex items-baseline justify-center md:justify-start">
                          ...
                          <span className="ml-2 text-2xl font-medium text-teal-600">
                            DT
                          </span>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="sticky top-0 flex flex-wrap border-b border-slate-200 bg-slate-50 z-10">
                {MASTER_CLASS_PACKS.map((pack) => (
                  <button
                    ...
                    className={`flex-1 py-3 px-2 text-center text-sm font-bold transition-all duration-200 focus:outline-none ${
              ...
              <div className="p-8 md:p-12">
                {MASTER_CLASS_PACKS.map((pack) => {
                  ...
                  const originalPriceHT = 240.0 * numberOfMasterClasses;

                  let features: string[] = [];
                  if (pack.id === 'MC_UNIT')
                    features = [
                      'Accès au cycle complet (3 sessions)',
                      'Support de cours PDF inclus',
                      'Replay disponible pendant 48h',
                      'Idéal pour se former sur un sujet précis',
                    ];
                  else if (pack.id === 'MC_PACK_3')
                    features = [
                      'Accès à 3 Master Classes complètes',
                      'Économie de ~15% sur le tarif unitaire',
                      'Supports de cours PDF inclus',
                      'Replay illimité sur les sessions choisies',
                      'Certificat de participation',
                    ];
                  else if (pack.id === 'MC_PACK_5')
                    features = [
                      'Accès à 5 Master Classes complètes',
                      'Économie de ~30% (Tarif préférentiel)',
                      'Supports de cours PDF inclus',
                      'Replay illimité',
                      'Accès prioritaire aux questions/réponses',
                    ];
                  else if (pack.id === 'MC_FULL')
                    features = [
                      'Accès INTÉGRAL aux 10 Master Classes',
                      'Tarif imbattable (-50%)',
                      'Bibliothèque complète de ressources',
                      'Accès à vie aux replays de la saison',
                      "Diplôme d'Honneur PharmIA",
                      'Statut VIP lors des événements',
                    ];
                    ...
                    >
                      <div className="flex-1 text-center md:text-left">
                        ...
                        <div className="mb-6 inline-block bg-teal-50 rounded-xl p-6 border border-teal-100">
                          ...
                          <p className="mt-2 text-xs text-slate-400 italic text-center md:text-left">
                            + 19% TVA et 1 DT de droit de timbre
                          </p>
                        </div>
