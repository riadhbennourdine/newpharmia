...
const EventCard: React.FC<{ event: PharmiaEvent }> = ({ event }) => (
  <Link to={`/events/${event.slug}`} className="block group">
    <div className="bg-white rounded-lg shadow-md overflow-hidden transition-shadow duration-300 hover:shadow-xl">
      ...
      <div className="p-6">
        <h2 className="text-xl font-bold text-gray-800 mb-2 group-hover:text-teal-600 transition-colors">{event.title}</h2>
        <p className="text-gray-600 text-sm mb-4">{event.summary}</p>
        <span className="font-semibold text-teal-700">Lire la suite &rarr;</span>
      </div>
    </div>
  </Link>
);
...
const EventsPage: React.FC = () => {
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8">
      <h1 className="text-4xl font-bold text-center mb-12 text-gray-800">Nos Événements</h1>
      {events.length > 0 ? (
        ...
        <p className="text-center text-gray-500">Aucun événement à afficher pour le moment.</p>
      )}
    </div>
  );
};
