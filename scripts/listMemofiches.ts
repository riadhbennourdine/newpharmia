...
async function listAllMemofiches() {
    ...
    try {
        console.log(`Starting to fetch all memofiches from ${API_BASE_URL}/memofiches`);
        ...
        while (currentPage <= totalPages) {
            ...
            const response = await fetch(`${API_BASE_URL}/memofiches?${queryParams.toString()}`);

            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Failed to fetch memofiches page ${currentPage}: ${response.status} ${response.statusText} - ${errorText}`);
            }
