...
const simulateCleanup = async () => {
  ...
  const protectedCount = await collection.countDocuments({
    group: protectedGroup,
  });
  console.log(
    `[SAFETY] There are ${protectedCount} webinars in protected group "${protectedGroup}". They will NOT be touched.\n`,
  );
  ...
  console.log(
    `[TARGET] Found ${toDelete.length} "old" webinars to DELETE in group "${targetGroup}":`,
  );
  ...
  const toKeep = await collection
    .find({ group: targetGroup, title: /^MC\d+/ })
    .countDocuments();
  console.log(
