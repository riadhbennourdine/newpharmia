...
if (!MONGO_URL) {
  console.error(
    "Erreur : La variable d'environnement MONGO_URL est manquante.",
  );
  process.exit(1);
}
...
async function generateRetrospectiveStats() {
  ...
  try {
    ...
    registrationsByMonth.forEach((item) => {
      console.log(
        `- ${item._id.year}-${String(item._id.month).padStart(2, '0')}: ${item.count} nouvelles inscriptions`,
      );
    });
    ...
    const topFiches = await usersCollection
      .aggregate([
        { $unwind: '$readFiches' },
    ...
    if (topFiches.length > 0) {
      ...
      topFiches.forEach((fiche, index) => {
        console.log(
          `${index + 1}. "${fiche.ficheDetails.title}" - ${fiche.count} lectures`,
        );
      });
    } else {
    ...
    console.error(
      'Une erreur est survenue lors de la génération des statistiques :',
      error,
    );
  } finally {
    await client.close();
