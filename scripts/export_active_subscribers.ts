...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://mongo:YoEfFXGVQwTTQlnwwPYRKwdIrgEqXrNp@centerbeam.proxy.rlwy.net:33803';
...
async function downloadFile(url: string, dest: string): Promise<boolean> {
    try {
        ...
        if (url.startsWith('/uploads')) {
            ...
            const volumePath = path.resolve('/data/uploads', url.replace('/uploads/', ''));
...
async function exportActiveSubscribers() {
    ...
    try {
        ...
        const exportFolder = path.join(OUTPUT_DIR, `active_subscribers_${timestamp}`);
        ...
        const csvRows = [];
        // Header row
        csvRows.push(['ID', 'Nom', 'Prénom', 'Email', 'Ville', 'Téléphone', 'Formule', 'Date Début', 'Date Fin', 'Dernière Commande', 'Montant', 'Preuve Paiement']);
        ...
        for (const user of pharmacists) {
            ...
            if (lastOrder && lastOrder.paymentProofUrl) {
                ...
                const safeName = `${user.lastName || 'Inconnu'}_${user.firstName || 'Inconnu'}_${lastOrder._id}${extension}`.replace(/[^a-z0-9._-]/gi, '_');
                const destPath = path.join(proofsFolder, safeName);
                
                const downloaded = await downloadFile(lastOrder.paymentProofUrl, destPath);
                if (downloaded) {
            ...
            const formatDate = (d: Date | string | undefined) => d ? new Date(d).toLocaleDateString('fr-FR') : 'N/A';
