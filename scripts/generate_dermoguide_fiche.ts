...
const dermoSchema = {
  ...
  properties: {
    id: {
      ...
      description: 'Slug unique (ex: psoriasis-vulgaire)',
    },
    ...
    group: {
      ...
      enum: ['A', 'B', 'C', 'D'],
      description:
        'A=Ça gratte, B=Boutons Visage, C=Plaques & Squames, D=Mains/Pieds',
    },
    shortDef: {
      type: SchemaType.STRING,
      description: 'Définition courte en 1 phrase',
    },
    pharma: {
      type: SchemaType.OBJECT,
      properties: {
        profil: {
          type: SchemaType.STRING,
          description: 'P - Terrain, Âge, Sexe',
        },
        histoire: {
          type: SchemaType.STRING,
          description: 'H - Facteurs déclenchants, début, contexte',
        },
        analyse: {
          type: SchemaType.OBJECT,
          properties: {
            lesionPrimaire: {
              type: SchemaType.STRING,
              description: 'Type précis (Macule, Papule...)',
            },
            aspectVisuel: {
              type: SchemaType.STRING,
              description:
                'Description visuelle détaillée (forme, couleur, texture)',
            },
            symptomes: {
              type: SchemaType.STRING,
              description: 'Prurit, douleur, brûlure',
            },
          },
        ...
        recurrence: {
          type: SchemaType.STRING,
          description: 'R - Chronique ou Aigu',
        },
        medicaments: {
          type: SchemaType.STRING,
          description: 'M - Lien iatrogène ou confusion',
        },
        alerte: {
    ...
    differential: {
      ...
      items: {
        ...
        properties: {
          ...
          description: {
            type: SchemaType.STRING,
            description: 'Pourquoi on confond et comment différencier',
          },
        },
      },
    },
    imagePrompt: {
      type: SchemaType.STRING,
      description:
        "Prompt précis pour générer/chercher une image d'atlas correspondante",
    },
  },
  required: [
    'id',
...
// --- Generation Logic ---
async function generateDermoFiche(
...
if (target) {
  ...
  console.log(
    'Usage: npx ts-node scripts/generate_dermoguide_fiche.ts <NomPathologie> [FichierContexteOptional]',
  );
  console.log(
    "Exemple: npx ts-node scripts/generate_dermoguide_fiche.ts 'Psoriasis'",
  );
}
