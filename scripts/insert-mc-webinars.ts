...
const insertMasterClasses = async () => {
  ...
  try {
    ...
    console.log(
      `Attempting to insert ${masterClassData.length} MasterClasses...`,
    );
    let insertedCount = 0;
    let skippedCount = 0;
    ...
    for (const mc of masterClassData) {
      ...
      if (!parsedDate) {
        console.error(
          `Skipping MasterClass "${mc.title}" due to invalid date format: ${mc.date}`,
        );
        skippedCount++;
        continue;
      }
      ...
      if (existingWebinar) {
        console.log(
          `MasterClass already exists, skipping insertion: "${newWebinar.title}" (${newWebinar.date.toLocaleDateString()})`,
        );
        skippedCount++;
      } else {
        ...
        if (result.acknowledged) {
          console.log(
            `Inserted: "${newWebinar.title}" (${newWebinar.date.toLocaleDateString()})`,
          );
          insertedCount++;
        } else {
    ...
    console.log(
      `Insertion complete. Successfully inserted ${insertedCount} new MasterClasses, skipped ${skippedCount}.`,
    );
  } catch (error) {
