...
async function run() {
  ...
  const angineSim = [...simulations].reverse().find((s: any) => 
    s.topic && s.topic.toLowerCase().includes('angine')
  );

  if (!angineSim) {
    console.log("Aucune simulation sur l'angine trouvée pour cet utilisateur.");
    console.log("Dernières simulations :", simulations.slice(-5).map((s:any) => s.topic));
    process.exit(0);
  }
