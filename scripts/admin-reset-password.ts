...
if (!MONGODB_URI) {
  console.error(
    "Erreur : La variable d'environnement MONGODB_URI est manquante.",
  );
  process.exit(1);
}
...
async function resetPassword() {
  ...
  if (!email) {
    console.error(
      'Usage: tsx scripts/admin-reset-password.ts <email> [nouveauMotDePasse]',
    );
    console.error(
      "Si aucun mot de passe n'est fourni, un mot de passe aléatoire sera généré.",
    );
    process.exit(1);
  }
  ...
  if (!newPassword) {
    ...
    console.log(
      "Aucun mot de passe fourni, génération d'un mot de passe aléatoire...",
    );
  }
