...
const clearMasterClasses = async () => {
    ...
    try {
        ...
        const cropCountBefore = await webinarsCollection.countDocuments({ group: protectedGroup });
        console.log(`[SAFETY CHECK] Found ${cropCountBefore} webinars in protected group "${protectedGroup}".`);
        ...
        const cropCountAfter = await webinarsCollection.countDocuments({ group: protectedGroup });
        
        if (cropCountBefore === cropCountAfter) {
             console.log(`[SUCCESS] Protected group "${protectedGroup}" remains untouched (${cropCountAfter} webinars).`);
        } else {
             console.error(`[WARNING] Mismatch in protected group count! Before: ${cropCountBefore}, After: ${cropCountAfter}`);
        }
