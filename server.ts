  ...
  */
  ...
  app.get('/api/proxy-pdf', async (req, res) => {
    try {
      ...
      if (!pdfUrl || typeof pdfUrl !== 'string') {
        return res
          .status(400)
          .json({ message: 'A valid pdfUrl is required.' });
      }
  ...
  app.post('/api/gemini/generate-dermo-fiche', async (req, res) => {
    try {
      ...
      if (!pathologyName) {
        return res
          .status(400)
          .json({ message: 'Pathology name is required.' });
      }
  ...
  app.post('/api/gemini/chat', authenticateToken, async (req, res) => {
    try {
      ...
      const history = messages.filter((m: any) => m.role !== 'model');
  ...
  app.post('/api/rag/chat', authenticateToken, async (req, res) => {
    try {
      ...
      const ficheObjectIDs = (algoliaResults as any).map(
      ...
      const context = fullFiches
        .map((fiche) => {
          return `Titre: ${fiche.title}\nContenu: ${extractTextFromMemoFiche(
            fiche,
          )}\n`;
        })
        .join('\n---\n');
  ...
  app.post('/api/gemini/dermo-patient', authenticateToken, async (req, res) => {
    try {
      ...
      if (!fiche) {
        ...
        return res
          .status(400)
          .json({ message: 'Fiche content is required.' });
      }
      const response = await getDermoPatientResponse(
        history,
        fiche,
        message,
      );
  ...
  app.get('/api/konnect/webhook', async (req, res) => {
    try {
      ...
      if (!response.ok) {
        console.error(
          `Konnect Webhook: Failed to verify payment ${paymentRef}`,
        );
  ...
  app.post('/api/gpg/initiate-payment', async (req, res) => {
    try {
      ...
      console.log(
        `[GPG Init] Password Length: ${ 
          GPG_PASSWORD ? GPG_PASSWORD.length : 0 
        }`,
      );
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (sendToExpired) {
          ...
          const confirmedAttendeeUserIds = (
            fetchedWebinar.attendees as any[]
          )
            .filter((att) =>
          ...
          if (formalGroupIds && formalGroupIds.length > 0) {
            ...
            groups.forEach((group: any) => {
              (group.pharmacistIds as ObjectId[]).forEach((id: ObjectId) =>
                userIdsFromGroups.add(id.toString()),
              );
              (group.preparatorIds as ObjectId[]).forEach((id: ObjectId) =>
                userIdsFromGroups.add(id.toString()),
              );
            });
        ...
        const validRecipients = recipients.filter((r: any) => {
          if (!r.email || !/\S+@\S+\.\S+/.test(r.email)) {
            console.warn(
              `Skipping recipient with invalid email: ${r.email || 'N/A'}, ID: ${ 
                r._id 
              }`,
            );
            return false;
        ...
        console.log(
          `Sending newsletter to ${validRecipients.length} valid recipients (skipped ${ 
        ...
        const emailMessages = validRecipients.map((recipient: any) => {
          ...
          if (webinarId && fetchedWebinar) {
            finalHtmlContent = contentWithUserId
              .replace(
                /{{LIEN_MEETING}}/g,
                (fetchedWebinar as any).googleMeetLink || '',
              )
              .replace(
                /{{WEBINAR_DESCRIPTION}}/g,
                (fetchedWebinar as any).description || '',
              );
          } else {
            finalHtmlContent = contentWithUserId;
          }
        ...
        await sendBulkEmails(emailMessages as any);
    ...
    async (req, res) => {
      try {
        ...
        if (webinarId) {
          ...
          if (webinar && (webinar as any).googleMeetLink) {
            finalHtmlContent = htmlContent.replace(
              /{{LIEN_MEETING}}/g,
              (webinar as any).googleMeetLink,
            );
          }
        }
        ...
        const sendPromises = testEmails.map((email: string) => {
          if (!/\S+@\S+\.\S+/.test(email)) {
