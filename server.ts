// Global error handlers to catch any unhandled exceptions or rejections
process.on('uncaughtException', (error) => {
  console.error('UNCAUGHT EXCEPTION:', error);
  // It's critical to exit the application after an uncaught exception
  // to prevent the application from continuing in a potentially corrupted state.
  process.exit(1);
});
...
process.on('unhandledRejection', (reason, promise) => {
  ...
  // In a production application, you might want to exit here as well,
  // or at least send a notification to a monitoring service.
});
