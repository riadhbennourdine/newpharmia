...
router.post('/', authenticateToken, checkRole([UserRole.ADMIN]), async (req, res) => {
    try {
        ...
        const result = await imageThemesCollection.insertOne(newTheme as ImageTheme);

        if (result.acknowledged) {
            res.status(201).json({ message: 'Image theme created successfully.', themeId: result.insertedId });
        } else {
        ...
        res.status(500).json({ message: 'Erreur interne du serveur lors de la création du thème d\'image.' });
    }
});
...
router.get('/', async (req, res) => {
    try {
        ...
        const db = client.db('pharmia');
        const imageThemesCollection = db.collection<ImageTheme>('imagethemes');
        const themes = await imageThemesCollection.find({}).sort({ name: 1 }).toArray();
        res.json(themes);
    } catch (error) {
        console.error('Error fetching image themes:', error);
        res.status(500).json({ message: 'Erreur interne du serveur lors de la récupération des thèmes d\'image.' });
    }
});
...
router.delete('/:id', authenticateToken, checkRole([UserRole.ADMIN]), async (req, res) => {
    try {
        ...
        const result = await imageThemesCollection.deleteOne({ _id: new ObjectId(id) });
