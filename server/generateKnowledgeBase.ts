...
function formatRichField(field: string | MemoFicheSection | undefined, titleFallback: string): string {
  if (!field) return '';
...
function formatMemoFiche(fiche: MemoFiche): string {
  ...
  md += formatRichField(fiche.patientSituation, 'Situation Patient / Cas Comptoir');
  ...
  if (fiche.recommendations) {
     md += formatListField(fiche.recommendations.mainTreatment, 'Traitement Principal (Rec)');
     md += formatListField(fiche.recommendations.associatedProducts, 'Produits Associés (Rec)');
     md += formatListField(fiche.recommendations.lifestyleAdvice, 'Conseils Hygiène de Vie (Rec)');
     md += formatListField(fiche.recommendations.dietaryAdvice, 'Conseils Alimentaires (Rec)');
  }
  ...
  md += formatRichField(fiche.dispositifsAConseiller, 'Dispositifs à Conseiller');
  ...
  if (fiche.conseilsTraitement) {
    ...
    if (Array.isArray(fiche.conseilsTraitement)) {
        ...
        if (typeof fiche.conseilsTraitement[0] === 'string') {
             md += (fiche.conseilsTraitement as string[]).map(t => `- ${t}`).join('\n');
        } else {
...
export async function generateKnowledgeBase() {
  ...
  try {
    ...
    const memofiches = await db.collection<MemoFiche>('memofiches').find({}).toArray();
