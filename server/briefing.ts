  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: 'Only pharmacists can generate briefings.' });
      }

      const { groupsCollection, webinarsCollection, memofichesCollection } =
      ...
      const weekendSeminars = await webinarsCollection
        .find({
      ...
      if (nextCrop) {
        ...
        actions.push({
          label: `📅 CROP : ${nextCrop.title}`,
          url: `/webinars/${nextCrop._id}`,
        });
      }
      ...
      if (nextMasterClass) {
        ...
        actions.push({
          label: `🎓 MasterClass : ${nextMasterClass.title}`,
          url: `/webinars/${nextMasterClass._id}`,
        });
      }

      let weekendText = undefined;
      if (weekendSeminars.length > 0) {
        weekendText = weekendSeminars
          .map(
            (w) =>
              `${w.title} (${new Date(w.date).toLocaleDateString('fr-FR', { weekday: 'long' })})`,
          )
          .join(', ');
      ...
      const randomFiche = await memofichesCollection
        .aggregate([
          { $sample: { size: 1 } },
      ...
      let tip = '';
      if (
        randomFiche.length > 0 &&
        randomFiche[0].keyPoints &&
        randomFiche[0].keyPoints.length > 0
      ) {
      ...
      if (group) {
        ...
        const groupUsers = await usersCollection
          .find(
            { _id: { $in: memberIds } },
        ...
        groupUsers.forEach((u) => {
          if (u.quizHistory && u.quizHistory.length > 0) {
            const weeklyQuizzes = u.quizHistory.filter(
              (q: any) => new Date(q.completedAt) >= oneWeekAgo,
            );
            const weeklyTotal = weeklyQuizzes.reduce(
      ...
      await groupsCollection.updateOne(
        ...
        {
          $set: {
            dailyBriefing: { script, date: new Date(), actions, audioUrl },
          },
        },
      );
      ...
      res
        .status(500)
        .json({ message: 'Erreur lors de la génération du briefing.' });
    }
  },
);
