    ...
    const clients = await usersCollection
      .aggregate([
        ...
        },
        {
          $lookup: {
            ...
            pipeline: [
              {
                $match: {
                  $expr: { $eq: ['$pharmacistId', '$$pharmacist_id'] },
                },
              },
              { $count: 'count' },
            ],
        ...
        },
        {
          $addFields: {
            teamSize: {
              $ifNull: [{ $arrayElemAt: ['$teamInfo.count', 0] }, 0],
            },
          },
        },
        {
...
router.get('/prospects', async (req, res) => {
  try {
    ...
    const db = client.db('pharmia');
    const usersCollection = db.collection<User>('users');
    const prospects = await usersCollection
      .find({
        status: ClientStatus.PROSPECT,
      })
      .toArray();
...
router.post('/appointments', async (req, res) => {
  try {
    ...
    if (!clientId || !date || !title) {
      return res
        .status(400)
        .json({ message: 'Client, date and title are required.' });
    }
