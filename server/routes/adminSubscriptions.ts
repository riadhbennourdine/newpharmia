...
router.get('/', authenticateToken, async (req: AuthenticatedRequest, res) => {
    ...
    try {
        ...
        const enhancedSubscribers = await Promise.all(subscribers.map(async (user) => {
...
router.post('/:userId/upload-proof', authenticateToken, upload.single('file'), async (req: AuthenticatedRequest, res) => {
    if (req.user?.role !== UserRole.ADMIN) {
        return res.status(403).json({ message: 'Unauthorized.' });
    }
    ...
    try {
        ...
        let order = await ordersCollection.findOne(
            { userId: new ObjectId(userId), status: { $in: [OrderStatus.PENDING_PAYMENT, OrderStatus.PAYMENT_SUBMITTED] } },
            { sort: { createdAt: -1 } }
        );
        ...
        res.json({ message: 'Proof uploaded and linked successfully.', fileUrl, orderId });
