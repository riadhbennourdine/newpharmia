...
router.get('/', async (req, res) => {
  try {
    ...
    const resourcePages = await resourcePagesCollection.find({}).sort({ createdAt: -1 }).toArray();
    res.json(resourcePages);
  } catch (error) {
    res.status(500).json({ message: 'Erreur serveur lors de la récupération des pages de ressources.' });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!ObjectId.isValid(id)) {
      return res.status(400).json({ message: 'ID de page de ressource invalide.' });
    }
    const resourcePagesCollection = await getCollection();
    const resourcePage = await resourcePagesCollection.findOne({ _id: new ObjectId(id) });
    if (!resourcePage) {
      return res.status(404).json({ message: 'Page de ressource non trouvée.' });
    }
    res.json(resourcePage);
  } catch (error) {
    res.status(500).json({ message: 'Erreur serveur lors de la récupération de la page de ressource.' });
  }
});
...
router.post('/', authenticateToken, checkRole([UserRole.ADMIN]), async (req, res) => {
  try {
    ...
    if (!title || !resources) {
        return res.status(400).json({ message: 'Le titre et les ressources sont requis.' });
    }
    ...
    const result = await resourcePagesCollection.insertOne(newResourcePage as ResourcePage);
    ...
    res.status(500).json({ message: 'Erreur serveur lors de la création de la page de ressource.' });
  }
});
...
router.put('/:id', authenticateToken, checkRole([UserRole.ADMIN]), async (req, res) => {
  try {
    ...
    const oldResourcePage = await resourcePagesCollection.findOne({ _id: new ObjectId(id) });
    ...
    const result = await resourcePagesCollection.updateOne({ _id: new ObjectId(id) }, { $set: updateData });
    ...
    res.status(500).json({ message: 'Erreur serveur lors de la mise à jour de la page de ressource.' });
  }
});
...
router.delete('/:id', authenticateToken, checkRole([UserRole.ADMIN]), async (req, res) => {
  try {
    ...
    const resourcePagesCollection = await getCollection();
    const resourcePageToDelete = await resourcePagesCollection.findOne({ _id: new ObjectId(id) });

    const result = await resourcePagesCollection.deleteOne({ _id: new ObjectId(id) });
    ...
    res.status(500).json({ message: 'Erreur serveur lors de la suppression de la page de ressource.' });
  }
});
