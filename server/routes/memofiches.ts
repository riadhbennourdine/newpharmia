      ...
      if (user) {
        ...
        if (user.groupId) {
          ...
          if (group && group.pharmacistIds && group.pharmacistIds.length > 0) {
            pharmacist = await usersCollection.findOne({
              _id: new ObjectId(group.pharmacistIds[0]),
            });
          }
        }
        ...
        if (!pharmacist && user.pharmacistId) {
          pharmacist = await usersCollection.findOne({
            _id: new ObjectId(user.pharmacistId),
          });
        }
      }
      ...
      const fichesWithAccess = fiches.map((fiche) => {
        ...
        if (user) {
          if (
            ...
            const hasPharmacistSubscription =
              subscriber &&
              subscriber.hasActiveSubscription &&
              subscriber.subscriptionEndDate &&
              new Date(subscriber.subscriptionEndDate) > new Date();
            ...
            if (
              ...
              hasPharmacistSubscription ||
              isInTrial ||
              (user.role === UserRole.PHARMACIEN && user.hasActiveSubscription)
            ) {
              hasAccess = true;
            }
            ...
            if (
              !hasAccess &&
              group &&
              group.assignedFiches.some(
                (f) => f.ficheId === fiche._id.toString(),
              )
            ) {
              hasAccess = true;
            }
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          const associatedPharmacist = await usersCollection.findOne({
            _id: new ObjectId(user.pharmacistId),
          });
          ...
          const groupDetails = await groupsCollection.findOne({
            _id: new ObjectId(user.groupId),
          });
          if (
            groupDetails &&
            groupDetails.pharmacistIds &&
            groupDetails.pharmacistIds.length > 0
          ) {
            const groupPharmacist = await usersCollection.findOne({
              _id: new ObjectId(groupDetails.pharmacistIds[0]),
            });
            if (groupPharmacist) {
        ...
        if (
          effectiveSubscriber &&
          ((effectiveSubscriber.hasActiveSubscription &&
            effectiveSubscriber.subscriptionEndDate &&
            new Date(effectiveSubscriber.subscriptionEndDate) > new Date()) ||
            (effectiveSubscriber.trialExpiresAt &&
              new Date(effectiveSubscriber.trialExpiresAt) > new Date()))
        ) {
          return res.json({
            ...fiche,
            isLocked: false,
  ...
  async (req, res) => {
    try {
      ...
      if (result.acknowledged) {
        ...
        res
          .status(500)
          .json({ message: "Échec de l'insertion de la mémofiche." });
      }
    } catch (error) {
      ...
      if (result.matchedCount === 0) {
        ...
        if (updatedFiche) {
          indexMemoFiches([updatedFiche]).catch((err) => {
            console.error(
              `Failed to re-index memofiche ${id} in Algolia:`,
              err,
            );
          });
        }
  ...
  async (req, res) => {
    try {
      ...
      if (result.deletedCount > 0) {
        ...
        const usersCollection = db.collection<User>('users');
        await usersCollection.updateMany({ 'readFiches.ficheId': id }, {
          $pull: { readFiches: { ficheId: id } },
        } as any);
        res.status(204).send();
      } else {
        res.status(404).json({ message: 'Mémofiche non trouvée' });
...
router.post('/validate-ids', async (req, res) => {
  try {
    ...
    const validFiches = await memofichesCollection
      .find(query)
      .project(projection)
      .toArray();
