...
const getBestModel = async (): Promise<string> => {
  ...
  try {
    ...
    // Priority list: Flash Stable (High Quota) > Flash 2.0 > Flash 2.5 (Strict Quota)
    const candidates = [
    ...
    console.warn('[Gemini] Could not auto-select model, falling back to gemini-flash-latest');
