...
const getBestModel = async (): Promise<string> => {
  ...
  try {
    ...
    // Priority list: Flash 1.5 (Fast & Cheap) > Pro 1.5 (Smart) > Pro 1.0 (Stable Legacy)
    const candidates = [
    ...
    console.warn('[Gemini] Could not auto-select model, falling back to gemini-pro');
    return 'gemini-pro';
...
export const refreshKnowledgeBaseCache = async (filePath: string) => {
  try {
    ...
    // If we are here, we are on a 1.5 model. But verify specifically if it's Flash 001 or Pro 001 which are usually the ones with cache.
    // However, let's try anyway if it's a 1.5 model.
