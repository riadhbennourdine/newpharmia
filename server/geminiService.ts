...
export const generateCaseStudyDraft = async (prompt: string, memoFicheType: string): Promise<Partial<CaseStudy>> => {
  ...
  if (memoFicheType === 'pharmacologie') {
    ...
    jsonStructure = {
      ...
      properties: {
        title: { type: Type.STRING },
        casComptoir: { type: Type.ARRAY, items: { type: Type.STRING } },
        objectifsConseil: { type: Type.ARRAY, items: { type: Type.STRING } },
        pathologiesConcernees: { type: Type.ARRAY, items: { type: Type.STRING } },
        interetDispositif: { type: Type.ARRAY, items: { type: Type.STRING } },
        beneficesSante: { type: Type.ARRAY, items: { type: Type.STRING } },
        dispositifsAConseiller: { type: Type.ARRAY, items: { type: Type.STRING } },
        reponsesObjections: { type: Type.ARRAY, items: { type: Type.STRING } },
        pagesSponsorisees: { type: Type.ARRAY, items: { type: Type.STRING } },
        references: { type: Type.ARRAY, items: { type: Type.STRING } },
      },
      required: [
        'title',
