...
router.put('/:userId/pharmia-credits', authenticateToken, checkRole([UserRole.ADMIN]), async (req, res) => {
    try {
        const { userId } = req.params;
        const { credits } = req.body;
        ...
        res.json({ message: 'User PharmIA credits updated successfully.', credits });

    } catch (error) {
        console.error('Error updating user PharmIA credits:', error);
        res.status(500).json({ message: 'Erreur interne du serveur lors de la mise à jour des crédits PharmIA.' });
    }
});
